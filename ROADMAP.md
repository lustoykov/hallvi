...
| 7 | Make reply execution inspectable through durable local history, structured logs, OpenTelemetry, and Langfuse, and keep application Activity to meaningful events. | Implemented in [PR #16](https://github.com/lustoykov/server-guy/pull/16), including the September 6 correction that separates application Activity from reply details. Extends alongside later Operations. [Checklist](#action-history-and-tracing), [small spec](docs/specs/action-history-and-tracing.md). |
...
- [x] Persist the accumulated assistant message with `body`, `status`, and a monotonically increasing `revision`; batch writes instead of storing one database row per token. Keep reply lifecycle/tool/retry facts in execution history; the application feed follows the [Activity inclusion rules](docs/specs/action-history-and-tracing.md#application-activity-inclusion-rules).
...
Give users application history for meaningful domain events and reply execution details for conversation diagnostics. The [action history and tracing spec](docs/specs/action-history-and-tracing.md#application-activity-inclusion-rules) owns inclusion rules and the flow inventory; build order and completion stay here. The first slice follows durable Pi requests and does not add a new Phase 1 acceptance gate.

- [x] Reuse Pi Run identity and durable reply execution history to correlate one chat execution across the worker, Pi, domain validation, database commit, and refreshed Operator View.
...
- [x] Initial implementation: expand every reply in Activity with timing, outcomes, and record/evidence links, plus optional authenticated Langfuse links. This placement was superseded by the September 6 product correction below; retain this as implementation history.
- [x] Verify the initial implementation's success, domain rejection after tool success, timeout/cancellation, restart/reconnect, unavailable telemetry, and payload-redaction scenarios. The corrected feed requires the separate verification below.
- [x] Apply the September 6 Activity inclusion rules: reply details sit collapsed under each Chat reply; per-reply and Chat creation/archive items are excluded from application Activity (historical Chat administration rows stay stored, unread); committed Decision, application creation, and repository verification events remain, with one requirement-saved event per Decision and one old → new event per replacement.
- [x] Record application repository-verification invalidation on a connection change/disconnect, once per invalidated Observation, without claiming unobserved access loss or repeating events on refresh, reads, or token renewal.
- [x] Verify the corrected feed and Chat placement with focused domain/browser cases: ordinary replies/lookups, Decision saves/replacements/rollback, cancellation, retry lineage, refresh, invalidation through the setup routes, and keyboard access to the reply details. The earlier per-reply Activity tests were replaced.
- [x] Separate reply diagnostics into `reply_execution_history`, keyed by Pi Run with cascade deletion. Upgrade schema 6 → 7 through the existing `db:push` command, preserving legacy payloads and missing-history behavior; retain application events in `activity_events`. Verify the upgrade, retry after rollback, reopening, pending-run recovery and cascades using disposable synthetic databases.
