...
2. Finish backup integration around native data/mount/writer facts, removing remaining service-name assumptions. PR #49 does this with declared owners and dump procedures for every database and a restore that boots the copy; its real-Pi evidence is BookStack with MariaDB and Healthchecks on the managed PostgreSQL. Capture and isolated restoration on the independently built shared-state topology, and legacy schedules on a real host, remain to be shown.
