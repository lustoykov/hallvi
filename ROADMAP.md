...
Reviewed 11 September 2026 against merged main `0682ab2` (PR #45), with the [BookStack follow-ups](docs/testing/2026-09-11-bookstack-followups.md) candidate (PR #49, under review) noted where it changes a row. These are implementation/evidence statements, not a claim that a particular local dashboard or remote application is currently running this revision.
...
| Verification limit | Initial intake still requires HTTP behavior checks and the primary host port 80. PR #49 adds command checks that run inside services with private inputs, record every check on the attempt, hold the deployment while a command's outcome is unknown, and run again inside a restored copy; background-only intake is still not implemented. |
...
1. Give Pi a small managed private-check capability that records actual execution and results, so useful behavior is not restricted to the current HTTP check shape. Keep observed runtime distinct from verified behavior. Command checks with recorded results, unknown-outcome holds and reuse in restore tests are in PR #49; an application without a public HTTP endpoint through the same initial/update path remains.
2. Finish backup integration around native data/mount/writer facts, removing remaining service-name assumptions. PR #49 does this with declared owners and dump procedures for every database and a restore that boots the copy and runs Pi's checks inside it; its real-Pi evidence is BookStack with MariaDB and Healthchecks on the managed PostgreSQL, each backed up and restore-tested with the owner's content proven in the copy. Capture and isolated restoration on the independently built shared-state topology, legacy schedules on a real host, and a controller update refreshing installed schedules by itself remain to be shown.
