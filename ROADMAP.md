...
- [x] Apply the September 6 Activity inclusion rules: reply details sit collapsed under each Chat reply; per-reply and Chat creation/archive items are excluded from application Activity (older rows stay stored, unread); committed Decision, application creation, and repository verification events remain, with one requirement-saved event per Decision and one old → new event per replacement.
