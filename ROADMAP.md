...
| --- | --- |
| Native deployment | PRs #43–#44 let Pi author native Compose and use the same managed execution/feedback loop for first deployments and updates. The custom primary/companion schema remains a historical compatibility reader. See [current deployment architecture](docs/architecture.md). |
...
Conversation-first integration and its legacy acceptance reconciliation are complete in the [dated integration evidence](docs/testing/README.md#dated-evidence). Preserve source/connection provenance, owner-merged candidate checks, durable cancellation/retry, populated migrations and history when changing the retained preparation path. Unreviewed model answers remain unreviewed. Remove remaining mandatory phase ceremony only while preserving applicable evidence and the narrow operability-PR boundary.
