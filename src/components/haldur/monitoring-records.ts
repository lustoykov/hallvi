...
export function monitoringFromRecords({
  ...
  if (!watcher)
    unwatched.push({
      ...
      short: "No watcher",
      detail:
        "Every result here is something Haldur looked at once. Nothing is checking on its own, so nothing would tell you if it stopped.",
    });
  for (const station of stations)
...
export type Usage = Extract<InformationContent, { kind: "usage" }> & {
  /** When Haldur read the window: its last bucket ends here. */
  at: string | null;
...
const checkWords: Record<string, [title: string, means: string]> = {
  ...
  ssh: ["Server accepts connections", "Haldur could log in over SSH."],
