...
async function jsonRequest<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (text) {
    try {
      ...
      if (response.ok)
        throw new Error("Haldur returned an unreadable response.");
    }
  }
  if (!response.ok) {
    throw new Error(
      body?.error ?? (text || "Haldur could not complete that request."),
    );
  }
  if (!body) throw new Error("Haldur returned an empty response.");
  return body;
}
