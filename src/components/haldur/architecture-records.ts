 ...
 * controller and the source repository have no subject kind: Haldur is
  ...
  if (!ref)
    return {
      ...
      detail: planned
        ...
        : part.kind === "controller"
          ? "This is Haldur, not something it observes."
 ...
 * where that belongs.
 */
/** A door on port 22 is the one Haldur comes through. */
function sshLooking(
...
export function architectureFromRecords({
  ...
  const parts: Part[] = map.value.parts.map((part) => {
    ...
    return {
      ...
      // Haldur and the repository have no state of their own to tag.
    ...
    [
      {
        ...
        summary:
          openness === "restricted"
            ? `A visit reaches ${headline} only from the machine running Haldur.`
            : openness === "public"
      ...
      },
      {
        ...
        summary: host
          ? `Haldur delivers to ${host.name} over the connection it holds.`
