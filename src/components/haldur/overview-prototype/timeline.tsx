...
// Haldur last checked, copied, reached the server and read the
...
function subline(model: HeroProps["model"], overview: Overview) {
  if (model.status !== "live")
    return model.status === "none"
      ? "Ask Haldur in the conversation to deploy it; this fills in as it runs."
      : "Nothing runs yet. Once you approve, Haldur builds it, checks it and starts copying its data off the server.";
...
export function TimelineHero({
  ...
  const [guy, mascot] = useHaldur(model, recheck, { narrate: true });
