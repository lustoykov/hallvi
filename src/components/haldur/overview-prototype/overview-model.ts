...
export function buildOverview({
  ...
  for (const part of model.parts) {
    ...
    needs.push({
      ...
      primary: {
        label: "Ask Haldur to look into it",
  ...
  const headline = planned
    ...
    : needs.length
      ...
      : model.condition.certainty === "stale"
        ? "Nothing needed you when Haldur last looked."
        : "Nothing needs you.";
  ...
  const vitals: Vital[] = [
    {
      ...
      lines: planned
        ? ["Haldur checks once it deploys"]
        : failing
          ? [`${failing.name} isn't answering`]
          : app?.evidence.certainty === "stale"
            ? [`${names} answered, nothing since`]
            : [`${names} answered`],
      plain: `Haldur checks ${app?.name ?? "the application"}'s health page${service ? ` and ${service.name}'s readiness` : ""} when it deploys and whenever you ask. Nothing checks in between.`,
    ...
    },
    {
      ...
      plain:
        "Every night Haldur copies the data off the server and checks the copy by size and checksum. A restore test proves a copy comes back.",
      facts: [],
      destination: "backups",
