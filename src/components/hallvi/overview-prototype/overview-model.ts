  ...
  const headline = planned
    ...
    : needs.length
      ...
      : model.condition.certainty === "stale"
        ? "Nothing needed you when Hallvi last looked."
        : "Nothing needs you.";
  ...
  const vitals: Vital[] = [
    {
      ...
      lines: planned
        ? ["Hallvi checks once it deploys"]
        : failing
          ? [`${failing.name} isn't answering`]
          : app?.evidence.certainty === "stale"
            ? [`${names} answered, nothing since`]
            : [`${names} answered`],
      plain: `Hallvi checks ${app?.name ?? "the application"}'s health page${service ? ` and ${service.name}'s readiness` : ""} when it deploys and whenever you ask. Nothing checks in between.`,
    ...
    },
    {
      ...
      plain:
        "Every night Hallvi copies the data off the server and checks the copy by size and checksum. A restore test proves a copy comes back.",
      facts: [],
      destination: "backups",
