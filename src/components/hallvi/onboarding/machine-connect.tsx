...
export function machineKeyLabel(publicKey: string) {
  ...
  const marker = encoded.replace(/[^A-Za-z0-9]/g, "").slice(0, 12);
...
export function machineCommand(publicKey: string) {
  ...
  return [
    ...
    `tmp=$(mktemp) && awk -v kind='${kind}' -v encoded='${encoded}' -v replacement='${taggedKey}' '$1 == kind && $2 == encoded { if (!seen++) print replacement; next } { print } END { if (!seen) print replacement }' ~/.ssh/authorized_keys > "$tmp" && cat "$tmp" > ~/.ssh/authorized_keys && rm -f "$tmp"`,
