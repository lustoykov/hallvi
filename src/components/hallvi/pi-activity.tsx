...
//   copy of your repository, and Hallvi's own records — and only the tool
...
import { hostOf, placeOf as placeOfTool, plainText } from "./execution-text";
...
export function PiActivity({
  ...
  return (
    <div className="hv-did" aria-label="What Pi did">
      {arrange(mine, (id) => cards.has(id)).map((item) =>
        item.type === "said" ? (
          <div className="hv-did-said" key={item.record.id}>
            <Markdown source={item.record.text ?? ""} />
          </div>
        ) : item.type === "card" ? (
          <div className="hv-did-card" key={item.record.id}>
      ...
      {tail && (
        <div className="hv-did-said" data-live="">
...
function Quiet({
  ...
  return (
    <div
      className="hv-did-quiet"
      data-open={open || undefined}
      data-wrong={failed || undefined}
    >
      <button
        type="button"
        className="hv-did-head"
        aria-expanded={open}
      ...
      >
        ...
        <span className="hv-did-marks" aria-hidden="true">
...
function Row({ record }: { record: ActivityRecord }) {
  ...
  return (
    <li className="hv-did-item" data-status={record.status}>
      <button
        type="button"
        className="hv-did-row"
        aria-expanded={open}
        onClick={() => setOpen(!open)}
      >
      ...
      {open && (
        <div className="hv-did-detail">
          ...
          {record.truncated && (
            <p>Longer than Hallvi keeps; what is shown was cut.</p>
          )}
        </div>
      )}
    </li>
