...
// Hallvi's own commands printed. Calling that "Logs" told an owner
// looking for their application's errors that they were in the right place.
//
// The output Hallvi already has, with the time it was captured. Nothing
...
function placeOf(tool: string) {
  return sharedPlaceOf(tool) ?? "Hallvi";
}
...
export function LogsPage({
  ...
  return (
    <div className="ax-root hv-section-page hv-section-logs">
      {bar}
      <header className="hv-section-header">
        <div>
          <h1>Command output</h1>
          <p>
            What Hallvi&rsquo;s own commands printed, with when it was captured.
            Not the application&rsquo;s logs: nothing here collects those, and
            nothing here runs anything. Each of these also sits on the event it
            belongs to in History.
          </p>
        </div>
      </header>
      <div className="hv-section-content">
        {streams.length > 0 && (
          <div className="hv-streams">
            <button
              type="button"
              className={`hv-stream${place === null ? " selected" : ""}`}
            ...
            {streams.map((stream) => (
              <button
                type="button"
                key={stream.label}
                className={`hv-stream${place === stream.label ? " selected" : ""}`}
        ...
        {shown.length === 0 && (
          <div className="hv-section-none">
            <h2>No output has been captured yet.</h2>
            <p>
              That is not a claim the application is quiet — only that nothing
              Hallvi ran has printed anything on record. If you are looking for
              the application&rsquo;s own logs, ask for them in the
        ...
        {matched.map((execution) => {
          ...
          return (
            <section className="hv-captured" key={execution.id}>
              ...
              <pre className="hv-logs-output">{lines.join("\n")}</pre>
            </section>
          );
        })}
      </div>
