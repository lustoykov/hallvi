...
export const SECRET_REQUEST_ANCHOR = "hv-secret-request";
...
export function SecretRequests({
  ...
  if (!waiting.length)
    return (
      <section
        className="hv-secrets"
        id={SECRET_REQUEST_ANCHOR}
        data-done=""
        aria-label="Values supplied to Hallvi"
      >
        <Who />
        <div className="hv-secrets-body">
          <details className="hv-secrets-receipt">
            <summary>
              <Check weight="bold" aria-hidden="true" />
              {supplied.length} configuration{" "}
              {supplied.length === 1 ? "value" : "values"} supplied
              <CaretRight
                className="hv-secrets-caret"
                weight="bold"
                aria-hidden="true"
              />
            </summary>
            <ul className="hv-secrets-given">
              {supplied.map((secret) => (
                <li key={secret.name}>
                  <code>{secret.name}</code>
                  {secret.process && <small>for {secret.process}</small>}
                  <span className="hv-secrets-when">
                    supplied {when(secret.establishedAt!)}
                  </span>
                </li>
              ))}
            </ul>
            <p className="hv-secret-note">
  ...
  return (
    ...
    >
      ...
      <div className="hv-secrets-body">
        ...
        <p className="hv-secrets-said" id={`${SECRET_REQUEST_ANCHOR}-said`}>
          ...
          <span>
            ...
            this conversation.
            {supplied.length > 0 && (
              <em className="hv-secrets-progress">
                {" "}
        ...
        <ul className="hv-secrets-list">
          ...
          {supplied.map((secret) => (
            <li key={secret.name} className="hv-secrets-done">
              <Check weight="bold" aria-hidden="true" />
              <code>{secret.name}</code>
              <span className="hv-secrets-when">
                supplied {when(secret.establishedAt!)}
        ...
        <details className="hv-secret-how">
          <summary>
            How Hallvi handles these
          ...
          <p>
            ...
            copy of the whole Hallvi configuration, or anyone who can read this
            computer.
          </p>
        </details>
      </div>
  ...
  return (
    <button
      ...
      onClick={() => {
        ...
        block.classList.add("hv-message-highlight");
        window.setTimeout(
          () => block.classList.remove("hv-message-highlight"),
          2600,
        );
        block
    ...
    >
      ...
      <span className="hv-secrets-chip-go">go to the request ↑</span>
    </button>
  );
}
  ...
  return (
    ...
    >
      <label htmlFor={field}>
        <code>{secret.name}</code>
        {secret.process && <small>for {secret.process}</small>}
        <span className="hv-secret-why" title={secret.why}>
          {purposeOf(secret.why)}
        </span>
      </label>
      <div className="hv-secret-row">
        <input
          id={field}
          type="password"
          autoComplete="off"
      ...
      {error && (
        <p className="hv-secret-error" role="alert">
          {error}
        </p>
      )}
      {short && (
        <p className="hv-secret-error" id={`${field}-short`}>
