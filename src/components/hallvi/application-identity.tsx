...
export function ApplicationIdentity({
  ...
  if (variant === "breadcrumb")
    return (
      <div className="hv-identity-breadcrumb">
  ...
  return (
    <div className={`hv-app-identity hv-identity-${variant}`}>
      <button
        ref={anchor}
        className="hv-identity"
        type="button"
        popoverTarget={menuId}
        disabled={disabled}
        aria-label={`Switch application: ${nameOf(application)}`}
      >
        <span className="hv-identity-mark" aria-hidden="true">
          {initial(application)}
        </span>
        ...
        <span className="hv-identity-text">
          <strong title={nameOf(application)}>{nameOf(application)}</strong>
          <small title={repository}>{repository}</small>
        </span>
      ...
      >
        <span className="hv-eyebrow hv-application-menu-label">
          Switch application
        </span>
        {applications.map((item) => (
          ...
          >
            <span aria-hidden="true" className="hv-application-menu-mark">
              {initial(item)}
            </span>
            <div>
        ...
        <Link className="hv-application-menu-action" href={addHref}>
