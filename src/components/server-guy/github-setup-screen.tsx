...
async function request<T>(
  ...
  if (!response.ok || !data)
    throw new Error(data?.error ?? "Could not reach Server Guy. Try again.");
  return data;
}
    ...
    <main className={`sg-setup-shell ${s.root}`}>
          ...
          <section className={s.section}>
            ...
            <p className={s.hint}>
              {status.connection?.mode === "app"
                ? "Choose which repositories Server Guy can read on GitHub. Existing applications are checked automatically after reconnecting."
                : connected
      ...
      >
        ...
        <p>
          ...
          operating-system user running Server Guy. Your password never reaches
          Server Guy.
        </p>
        <code>{status.storagePath}</code>
        <p>
          Access is renewed automatically when needed for a GitHub request. If
          renewal expires or is rejected, sign in again. Server Guy does not
          ship a GitHub App private key or client secret.
        </p>
        <h3>Permissions</h3>
        <p>
