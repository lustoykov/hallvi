...
export function GithubSetupScreen({ initialStatus, returnToAdd = false }: { initialStatus: GithubSetupStatus; returnToAdd?: boolean }) {
  ...
  return <main className={`sg-setup-shell ${s.root}`}>
      ...
      <section className={s.card}>
        <section className={s.section} aria-labelledby="github-account-heading">
          ...
          {working && attempt ? <div className={s.device}>
            ...
            {status.connection.expiresAt && <p className={s.connectionHelp}>{status.connection.automaticRenewal ? "Access renews automatically." : "Sign in again to enable automatic renewal."}</p>}
    ...
    <aside popover="auto" id="github-storage" className={s.help} aria-labelledby="github-storage-title">
      ...
      <h3>Separate login</h3><p>GitHub sign-in stores access and refresh tokens in the file below. They are not encrypted; the file is readable and writable only by the operating-system user running Server Guy. Your password never reaches Server Guy.</p><code>{status.storagePath}</code>
      <p>Access is renewed automatically when needed for a GitHub request. If renewal expires or is rejected, sign in again. Server Guy does not ship a GitHub App private key or client secret.</p>
