    ...
    <main className={s.page}>
      ...
      <section className={`${s.content} ${s.newContent}`} aria-labelledby="new-application-heading">
        ...
        <form className={s.form} onSubmit={(event) => { event.preventDefault(); void createApplication(); }}>
          ...
          <div className={s.actions}><Link href="/applications">{busy ? "Back to applications" : "Cancel"}</Link><button className={s.primary} disabled={busy || !repositoryUrl.trim()} type="submit">{busy && <SpinnerGap className="spin" />}{busy ? "Checking repository…" : "Add application"}</button></div>
        </form>
      </section>
    </main>
  );
