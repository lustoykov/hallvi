...
async function readJson<T>(response: Response): Promise<T> {
  ...
  if (!response.ok || !body)
    throw new SetupRequestError(
      body?.error ?? "Could not reach Server Guy. Try again.",
      response.status,
    );
  return body as T;
}
    ...
    <main className={"sg-setup-shell " + s.root}>
      ...
      >
        ...
        <p>
          ...
          Server Guy (0600). Existing files keep their permissions.
        </p>
        <p>
          The tokens are not encrypted by Pi or Server Guy or stored in an OS
        ...
        <h3>Login files</h3>
        <p>
          Stored on the machine running Server Guy—not necessarily this
          browser’s computer.
        </p>
        {status.hasSavedConfiguration && (
          <>
        ...
        <p>
          ...
          model changes apply only to Server Guy. A new login uses its own file
          and replaces your current connection only after it succeeds.
        </p>
        <p>
        ...
        <h3>Disconnecting</h3>
        <p>
          Disconnect removes Server Guy’s saved connection choice and model
