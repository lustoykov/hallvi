...
const deciders = [
  ...
  },
  {
    ...
    note: "Held on the host. Server Guy never shows one back, here or anywhere.",
  },
  {
    key: "generated",
...
export function ManifestDirection({
  ...
  const say = story.waiting.length
    ? `Server Guy is waiting for ${listed(story.waiting.map((item) => item.name))}.`
    ...
    <section className="axma" aria-label="Environment variables">
      ...
      {story.waiting.length > 0 && (
        <div className="axma-waiting" role="status">
          ...
          <p>
            Server Guy asks for these in the conversation that needs them, and
