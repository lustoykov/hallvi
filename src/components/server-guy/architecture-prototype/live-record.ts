...
// Reads the live record from the app's own API, GET requests only, and falls
// back to what the page already has when that read fails.
...
export function useLiveRecord(applicationId: string, fallback: LiveRecord) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/applications/${applicationId}/firewall`, {
      cache: "no-store",
