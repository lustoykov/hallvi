...
export function productName(image: string | null | undefined, fallback: string) {
...
export function buildModel({
  ...
  const images = [plan?.image, ...(plan?.services ?? []).map((s) => s.image)]
  ...
  parts.push({
    ...
    facts: [
      ...
      deployment?.address
        ...
        : null,
      offer ? { label: "Location", value: `${region} (${offer.location})` } : null,
      offer
  ...
  for (const volume of stack.volumes) {
    ...
    parts.push({
      ...
      evidence,
      destination: sqlite || volume.kind === "database" ? "database" : "storage",
    });
  }
...
export function explain(model: ArchitectureModel, depth: Depth): Sentence[] {
  ...
  const offsiteReal = p.offsite.evidence.certainty !== "absent" &&
  ...
  if (depth === 1) {
    ...
    say(
      ...
      offsiteReal && `; the last one was checked by checksum ${when("offsite")}`,
      ".",
    );
    say(
      "sure",
  ...
  if (service)
    say(
      "private",
      { ref: service.id, text: fact(service.id, "Image") ?? service.name, mono: true },
      " answers only as ",
      { text: fact(service.id, "Inside the server") ?? service.name, mono: true },
