  ...
  return (
    <div className="ax-bar" role="region" aria-label="Prototype controls">
      ...
      <div className="ax-bar-variant">
        <button type="button" onClick={() => step(-1)} aria-label="Previous direction">
        ...
        <button type="button" onClick={() => step(1)} aria-label="Next direction">
      ...
      <span className={`ax-bar-source${current.invented ? " is-invented" : ""}`}>
        {current.invented
          ? `Invented · ${current.note}`
          : source === "live"
            ? "Live record from :3270"
            : source === "loading"
              ? "Reading the record…"
