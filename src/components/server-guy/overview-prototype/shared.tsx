...
/** "today at 03:00", "yesterday at 15:03", "tomorrow at 03:00", "on 7 Sep at 10:00". */
  ...
  return (
    ...
    >
      ...
      <div className="axo-need-actions">
        {need.partId ? (
          <button type="button" className="ax-textlink" onClick={() => onShow(need.partId!)}>
            Show on the map
  ...
  return (
    <div className="axo-idea">
      ...
      <div className="axo-idea-actions">
        <button type="button" className="ax-textlink" onClick={() => onAsk(idea.draft)}>
...
export function OpChip({ state }: { state: OperationState }) {
  const look: Record<OperationState, { tone: string; icon: ReactNode }> = {
    ...
    working: { tone: "checking", icon: <SpinnerGap weight="bold" className="ax-spin" /> },
...
export function VitalPop({
  ...
  return (
    <div className="axj2-pop axo-pop-card" role="dialog" aria-label={vital.label}>
      <header>
