...
function titleOf(lane: LaneId, tone: EventTone, moments: Moment[]) {
  ...
  switch (lane) {
    ...
    case "backups": {
      ...
      return restore && copy ? "Copy and restore test" : restore ? "Restore test" : "Copy verified";
    }
    case "server":
      return tone === "fail" ? "Couldn't reach the server" : "Reached the server";
    case "access":
...
function gather(lane: LaneId, moments: Moment[]): TimeEvent[] {
  ...
  for (const moment of [...moments].sort((a, b) => a.at - b.at)) {
    ...
    if (group && last && last.tone !== "planned" && moment.tone !== "planned" && moment.at - last.at < TOGETHER)
      group.push(moment);
...
export function buildTimeline({
  ...
  const moments: Record<LaneId, Moment[]> = { checks: [], backups: [], server: [], access: [] };
  const add = (
    lane: LaneId,
    ...
    moments[lane].push({
      ...
      line: {
        id,
        at: at!,
        tone: tone === "pass" ? "pass" : tone === "fail" ? "fail" : tone === "checking" ? "work" : "info",
        text,
        invented,
      },
    });
  ...
  if (model.status === "live") {
    ...
    add("server", record.deployment?.verifiedAt, "pass", "The server answered during the deployment", "deploy:host");
    ...
    for (const entry of protection?.history ?? []) {
      ...
      add(
        "backups",
        entry.at,
        entry.outcome === "succeeded" ? "pass" : entry.outcome === "failed" ? "fail" : "info",
        `${name}: ${entry.detail}`,
        `protection:${entry.id}`,
      );
    }
    ...
    if (next && protection?.policy)
      add("backups", next, "planned", `Scheduled: ${protection.policy.schedule} (${protection.policy.timezone})`, "next");

    // The firewall, as Hetzner reported it on this visit.
    const gate = model.byId["gate:http"];
    if (gate?.evidence.at && gate.evidence.short.startsWith("Read") && !gate.evidence.invented)
    ...
    for (const [partId, mark] of Object.entries(marks)) {
      ...
      moments[laneOf(partId)].push({
        at: now,
        tone: mark === "checking" ? "checking" : mark === "passed" ? "pass" : "fail",
        line: newest,
      });
    }
  }

  const lanes = (["checks", "backups", "server", "access"] as const).map((id) => {
    const past = moments[id].filter((moment) => moment.at <= now && moment.tone !== "planned");
    return {
      id,
      events: gather(
        id,
        moments[id].filter((moment) => moment.at >= start && moment.at <= end),
      ),
      lastAt: past.length ? Math.max(...past.map((moment) => moment.at)) : null,
    };
  });
