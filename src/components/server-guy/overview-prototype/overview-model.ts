...
export function buildOverview({
  ...
  const openOrigin = (operation: ApplicationOperation) =>
    operation.origin
      ? () =>
          onOpenConversation(operation.origin!.chatId, operation.origin!.messageId)
      : null;
  ...
  for (const part of model.parts) {
    ...
    needs.push({
      ...
      secondary: part.destination
        ? { label: `Open ${labelOf(part.destination)}`, destination: part.destination }
        : undefined,
    });
  }
  if (!planned) {
  ...
  const counts = ["", "One thing needs", "Two things need", "Three things need"];
  ...
  const checked = [app, service].filter((part): part is NonNullable<typeof part> =>
    Boolean(part),
  );
  ...
  const vitals: Vital[] = [
    ...
    },
    {
      ...
      facts: [
        ...
        protection?.policy && { label: "Keeps", value: protection.policy.retention },
    ...
    },
    {
      ...
      status: {
        ...
        text: host?.checking ? "Checking…" : (host?.evidence.short ?? "Planned"),
      },
      lines: [
        offer
      ...
      facts: [
        ...
        ...(offer
          ? [{ label: "Price", value: `${money(offer.monthly, offer.currency)} a month` }]
          : []),
      ],
    ...
    },
    {
      id: "access",
      label: "Access",
      value: planned ? "After deployment" : model.restricted ? "Your network only" : "Anyone",
      status: {
        certainty: gate?.evidence.certainty ?? "unknown",
        text: gate?.checking ? "Checking…" : (gate?.evidence.short ?? "Not observed"),
      },
      lines: [
        model.byId.tls?.evidence.certainty === "absent" ? "HTTP · no HTTPS yet" : "HTTPS",
