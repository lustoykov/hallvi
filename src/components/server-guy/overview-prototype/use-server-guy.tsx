...
export function useServerGuy(
  model: ArchitectureModel,
  recheck: Recheck,
  { narrate = false }: { narrate?: boolean } = {},
): [ServerGuy, RefObject<HTMLDivElement | null>] {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLive((previous) =>
        starting
          ? add
          : [...previous, ...add.filter((line) => !previous.some((old) => old.id === line.id))],
      );
  ...
  useEffect(() => {
    ...
    if (recheck.phase === "passed") {
      ...
      at(
        () =>
          burstAt(mascot.current, { count: 8, spread: 30, palette: sparklePalettes.failed }),
        120,
      );
    }
  ...
  useEffect(() => {
    ...
    element.animate(
      [{ translate: "0 0" }, { translate: "0 -12px", offset: 0.4 }, { translate: "0 0" }],
  ...
  const failedPart = model.parts.find((part) => part.evidence.certainty === "failed");
...
export function Mascot({
  ...
  return (
    ...
    >
      <LittleServer color={3} mood={guy.mood} gesture={guy.gesture} paused={guy.reduced} />
      {guy.bubble && (
