...
export function validatePiSelection(
  ...
  if (selection.providerId !== PI_PROVIDER_ID) {
    throw new Error(
      "ChatGPT subscription access only. Connect ChatGPT to continue.",
    );
  }
  const model = catalog.getModel(selection.providerId, selection.modelId);
  if (!model)
    throw new Error(
      "This model is not in the bundled Pi catalog. Custom model/provider definitions are not imported; choose an available ChatGPT model.",
    );
  if (
