import type {
  ConnectionAction,
  ConnectionItem,
} from "@/components/haldur/connections-screen";
...
function inApplication(
  ...
  if (applications.length === 0)
    return {
      ...
      note: "There are no applications yet, and Haldur will not choose one for you. Add one and this becomes part of its conversation.",
    };
...
/**
 * The Connections inventory: one row per account Haldur acts through,
  ...
  return [
    {
      ...
      detail: facts.hetznerConnected
        ? "A controller-held API token. Haldur chooses sizes and regions and shows the cost before buying."
        : "Not connected. Haldur cannot prepare a server without it.",
    ...
    // credentials, and Cloudflare issues them separately.
    {
      ...
      name: "Backup storage",
      purpose:
        "Writing backup copies into a bucket: your application's data, and Haldur's own records and keys.",
      state: storage.connected ? "connected" : "not-connected",
      detail: storage.connected
        ? `${storage.provider === "r2" ? "Cloudflare R2" : "Amazon S3"} at ${storage.host}. Haldur copies its own records here automatically; application backups are arranged in the conversation.`
        : cloudflare.connected
