...
Architecture reads particular keys, so use these whenever the evidence gives them: on a host, the ssh check and the address, region, size, server-id and os facts; on a web or private process, the http and container checks and the image, port and revision facts; on a volume, the persistence check and the path fact; on a door, the refused or open check and the port and sources facts; on a certificate, the valid check and the expires fact; on a monitor, the answering check and the target fact. A key outside that list is still saved and still readable as a detail, but it does not decide what a page says about a part.

For the application's map, put presentation.content={kind:"topology",from:"observed"|"plan",parts:[{id,kind,name,role,plain}],edges:[{from,to,network:"public"|"private"|"loopback"|"disk",label?}],absent:[{id,kind,name,would}]} on a record that states the application. Part kinds are controller, source, gate, tls, host, web, private, volume, offsite and monitor. Parts carry no state, because a part's state is the newest record stating that part: draw the shape here and record the state where it belongs. from:"plan" is the map before anything has run, so a reader can see the intended shape in ghost. absent is what is missing and what it would do if it were there, which is how a reader sees a gap instead of a tidy diagram. Use loopback for something reachable only through the host's own loopback, which is neither public nor the container network.
