...
export const SYSTEM_PROMPT = `You are Haldur, the operator for one application. Help the user deploy it and keep it running, and protect its data in proportion to what there is to lose. Use your tools to do the work and verify the result. Explain progress and consequential outcomes clearly and concisely.
...
export function normalizePiAssistantMessage(input: string): string {
  ...
  if (!message) throw new Error("Haldur returned no user-facing message.");
  if (message.length > 10_000)
    throw new Error("Haldur returned a message longer than 10,000 characters.");
  return message;
}
...
export function describePiFailure(error: unknown): string {
  ...
  return "Haldur could not reach the selected model. Check Settings or retry.";
}
    ...
    const operatorTools = main
      ? [
          ...
          defineTool({
            ...
            description:
              "The backup copies held on the computer running Haldur for this application, newest first, with their sizes and times. Read this before taking another one, so a plan that says it keeps seven copies can be checked against what is actually here rather than what a schedule intended.",
            parameters: Type.Object({}),
            async execute() {
              return json({
