...
export async function checkPublicAccess(
  ...
  const reading: PublicAccessReading = {
    ...
    checkedFrom:
      "The PC running Server Guy, over its own internet connection and its own DNS resolvers.",
    name: null,
    dns: null,
    expectAddress,
  ...
  if (params.url) {
    ...
    try {
      const response = await fetch(httpsUrl, {
        ...
        headers: { "user-agent": "Server Guy public access check" },
      });
...
export async function publicUrlReachable(url: string) {
  try {
    ...
    const response = await fetch(target, {
      ...
      headers: { "user-agent": "Server Guy access check" },
    });
    await response.body?.cancel();
