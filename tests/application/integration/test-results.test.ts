...
const roots: string[] = [];
function fixture() {
  const root = mkdtempSync(join(tmpdir(), "sg-results-test-"));
  roots.push(root);
...
it("keeps human and advisory LLM verdicts separate, without mutating the original report", () => {
  ...
  saveReview(root, run, hash, "greeting:1", {
    type: "human",
...
it("rejects stale results, fabricated cases and path traversal", () => {
  ...
  expect(() => findCase(root, run, "old-hash", "greeting:1")).toThrow(
    "Results changed",
  );
  expect(() => findCase(root, run, hash, "fabricated:1")).toThrow();
  expect(
    humanReviewSchema.safeParse({ reviewer: "", verdict: "pass", reason: "" })
      .success,
  ).toBe(false);
});
...
it("preserves run counts and distinguishes planned cases from available answers", () => {
  const { root, path } = fixture();
  writeJson(join(path, "results.json"), {
    ...readJson(join(path, "results.json")),
    caseIds: ["greeting", "hypothetical"],
    repeats: 2,
    plannedCases: 4,
  });
  expect(listReports(root)[0]).toMatchObject({
    caseIds: ["greeting", "hypothetical"],
    repeats: 2,
    plannedCases: 4,
  });
...
it("validates the whole bulk selection before writing human verdicts", () => {
  ...
  expect(() =>
    saveHumanReviews(root, run, hash, ["greeting:1", "unknown:1"], review),
  ).toThrow();
  expect(() =>
    saveHumanReviews(root, run, "stale", ["greeting:1"], review),
  ).toThrow();
  expect(listReports(root)[0].reviews).toEqual([]);
  expect(
    saveHumanReviews(root, run, hash, ["greeting:1", "greeting:2"], review),
  ).toEqual({ saved: ["greeting:1", "greeting:2"], failed: [] });
  expect(listReports(root)[0].reviews.map((r) => r.type)).toEqual([
    "human",
    "human",
  ]);
  expect(loadReport(root, run).hash).toBe(hash);
...
it("rejects invalid run archival and ignores metadata for a different source hash", () => {
  ...
  expect(() => archiveRun(root, "unknown", hash, true)).toThrow(
    "Unknown eval run",
  );
  ...
  writeJson(join(path, "run-state.json"), {
    sourceHash: "old-results",
    archived: true,
  });
  expect(listReports(root)[0].archived).toBe(false);
});
...
it("can archive failed or empty runs without reviewable answers", () => {
  const { root, run, path } = fixture();
  const original = readJson(join(path, "results.json"));
  original.results[0].reply = null;
  original.results[0].input = null;
  original.results[0].outcome = "not-run";
  writeJson(join(path, "results.json"), original);
  expect(archiveRun(root, run, loadReport(root, run).hash, true)).toEqual({
    archived: true,
  });
