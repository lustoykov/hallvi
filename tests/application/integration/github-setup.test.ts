...
import { connectedGithubCredential, credentialFingerprint, currentGithubConnectionId, detectGithubCliLogin, githubConnectionPath, invalidateGithubConnection, readGithubConnection, saveGithubConnection } from "../../../src/server/github-connection";
...
const tokenResponse = { access_token: "ghu_private-access-token", token_type: "bearer", expires_in: 28800, refresh_token: "ghr_private-refresh-token", refresh_token_expires_in: 15897600 };
const renewedResponse = { ...tokenResponse, access_token: "ghu_renewed-private-token", refresh_token: "ghr_renewed-private-token" };
...
describe("GitHub device flow", () => {
  it("preserves the old login until successful authorization and returns only public attempt fields", async () => {
    ...
    expect(readGithubConnection()).toMatchObject({ refresh: { token: tokenResponse.refresh_token, expiresAt: new Date(now + 15897600_000).toISOString() } });
    expect(statSync(githubConnectionPath()).mode & 0o777).toBe(0o600);
  });
...
describe("GitHub access renewal", () => {
  ...
  it("rotates both tokens without changing consent identity or exposing secrets", async () => {
    ...
    expect((await getGithubSetupStatus()).connection?.automaticRenewal).toBe(true);
    expect(device).not.toHaveBeenCalled(); // Reading Settings does not spend a refresh token.
    cli.mockClear();
    const credential = await connectedGithubCredential();
    expect(credential.token).toBe(renewedResponse.access_token);
    expect(credential.connection).toMatchObject({ id: previous.id, account: previous.account, connectedAt: previous.connectedAt,
      expiresAt: new Date(now + 28800_000).toISOString(), refresh: { token: renewedResponse.refresh_token, expiresAt: new Date(now + 15897600_000).toISOString() } });
    expect(device).toHaveBeenCalledExactlyOnceWith("/login/oauth/access_token", { client_id: "Iv1.test", grant_type: "refresh_token", refresh_token: tokenResponse.refresh_token });
    ...
    for (const secret of [tokenResponse.access_token, tokenResponse.refresh_token, renewedResponse.access_token, renewedResponse.refresh_token]) expect(publicState).not.toContain(secret);
    await connectedGithubCredential();
    expect(device).toHaveBeenCalledTimes(1);
  });

  it("renews just before expiry and uses the newly persisted refresh token next time", async () => {
    await appLogin(); device.mockClear().mockResolvedValue(renewedResponse);
    vi.setSystemTime(now += 28_700_000);
    await connectedGithubCredential(); expect(device).not.toHaveBeenCalled();
    vi.setSystemTime(now += 50_000);
    await connectedGithubCredential(); expect(device).toHaveBeenCalledTimes(1);
    vi.setSystemTime(now += 28_800_000);
    device.mockResolvedValue({ ...renewedResponse, access_token: "ghu_third", refresh_token: "ghr_third" });
    expect((await connectedGithubCredential()).token).toBe("ghu_third");
    expect(device).toHaveBeenLastCalledWith("/login/oauth/access_token", expect.objectContaining({ refresh_token: renewedResponse.refresh_token }));
  });
  ...
  it("shares one single-use exchange across simultaneous requests", async () => {
    ...
    device.mockImplementation(() => new Promise((done) => { resolve = done; }));
    const first = connectedGithubCredential(); const second = connectedGithubCredential();
    expect(device).toHaveBeenCalledTimes(1);
    resolve(renewedResponse);
    const results = await Promise.all([first, second]);
    expect(results[0]).toEqual(results[1]);
    expect(results[0].token).toBe(renewedResponse.access_token);
    expect(globalThis.__serverGuyGithubRefreshes?.has(githubConnectionPath())).toBe(false);
  });

  it.each(["disconnect", "replace", "invalidate"])("a pending refresh cannot undo %s", async (operation) => {
    const previous = await expiringLogin();
    let resolve!: (value: unknown) => void;
    device.mockImplementation(() => new Promise((done) => { resolve = done; }));
    const request = connectedGithubCredential();
    const rejected = expect(request).rejects.toThrow("changed during renewal");
    if (operation === "disconnect") disconnectGithub();
    if (operation === "replace") await reuse();
    if (operation === "invalidate") invalidateGithubConnection(previous, "Authorization revoked");
    const retained = readGithubConnection();
    resolve(renewedResponse);
    await rejected;
    expect(readGithubConnection()).toEqual(retained);
  ...
  it("a late failed refresh cannot invalidate a replacement connection", async () => {
    ...
    resolve({ error: "bad_refresh_token", error_description: "private-provider-data" });
    await rejected;
  ...
  it.each(["bad_refresh_token", "expired_token", "invalid_grant", "access_denied"])("requires sign-in after %s without leaking provider details or falling back to CLI", async (error) => {
    await expiringLogin();
    device.mockResolvedValue({ error, error_description: tokenResponse.refresh_token });
    await expect(connectedGithubCredential()).rejects.toThrow("Sign in again");
    expect(currentGithubConnectionId()).toBeNull();
    expect((await getGithubSetupStatus()).issue).not.toContain(tokenResponse.refresh_token);
    cli.mockClear();
    await expect(connectedGithubCredential()).rejects.toThrow("Sign in again");
    expect(device).toHaveBeenCalledTimes(1); expect(cli).not.toHaveBeenCalled();
  });

  it.each(["network", "provider"])("keeps credentials on a transient %s failure so the next request can retry", async (failure) => {
    const previous = await expiringLogin();
    if (failure === "network") device.mockRejectedValue(new api.GithubAccessError("Could not reach GitHub"));
    else device.mockResolvedValue({ error: "temporarily_unavailable", error_description: tokenResponse.refresh_token });
    await expect(connectedGithubCredential()).rejects.toBeInstanceOf(api.GithubAccessError);
    expect(readGithubConnection()).toEqual(previous);
    expect(currentGithubConnectionId()).toBe(previous.id);
    device.mockResolvedValue(renewedResponse);
    expect((await connectedGithubCredential()).token).toBe(renewedResponse.access_token);
  });
  ...
  it("rejects an incomplete rotated credential instead of saving half a token pair", async () => {
    ...
    await expect(connectedGithubCredential()).rejects.toThrow("incomplete login");
    expect(currentGithubConnectionId()).toBeNull();
    expect(readFileSync(githubConnectionPath(), "utf8")).not.toContain(renewedResponse.access_token);
  });

  it("never returns a rotated token unless its atomic file replacement succeeds", async () => {
    const previous = await expiringLogin();
    vi.mocked(renameSync).mockImplementationOnce(() => { throw new Error("private-filesystem-details"); });
    await expect(connectedGithubCredential()).rejects.toThrow("Could not save GitHub settings");
    expect(readGithubConnection()).toEqual(previous);
    expect(globalThis.__serverGuyGithubRefreshes?.has(githubConnectionPath())).toBe(false);
  });
  ...
  it("does not try an expired refresh token or discard still-valid access prematurely", async () => {
    const connection = await appLogin();
    if (connection.mode !== "app") throw new Error("Expected App login");
    saveGithubConnection({ ...connection, refresh: { token: tokenResponse.refresh_token, expiresAt: new Date(now - 1).toISOString() } });
    device.mockClear();
    await connectedGithubCredential(); expect(currentGithubConnectionId()).toBe(connection.id);
    vi.setSystemTime(now += 28_800_001);
    await expect(connectedGithubCredential()).rejects.toThrow("expired");
    expect(currentGithubConnectionId()).toBeNull(); expect(device).not.toHaveBeenCalled();
  });

  it("supports explicitly non-expiring App tokens without trying renewal", async () => {
    device.mockImplementation(async (path) => path === "/login/device/code" ? deviceCode : { access_token: "ghu_nonexpiring", token_type: "bearer" });
    const connection = await appLogin(); device.mockClear();
  ...
  it("uses the renewed token for repository verification and preserves the evidence connection ID", async () => {
    ...
    expect(result).toMatchObject({ status: "passed", raw: { connectionId: previous.id, accountId: account.id } });
    expect(json.mock.calls.every(([, accessToken]) => accessToken === renewedResponse.access_token)).toBe(true);
  ...
  it("retries a repository check once if another request rotated the token it was already using", async () => {
    ...
    let entered!: () => void;
    const oldRequestStarted = new Promise<void>((resolve) => { entered = resolve; });
    ...
    device.mockResolvedValue(renewedResponse);
    await connectedGithubCredential();
    rejectOldRequest(new api.GithubAccessError("Old token invalidated by rotation", "auth"));
    expect(await check).toMatchObject({ status: "passed", raw: { connectionId: previous.id } });
    expect(currentGithubConnectionId()).toBe(previous.id);
    expect(json.mock.calls.filter(([path]) => path === "/user")).toHaveLength(3); // login + old request + retry
  });
});
