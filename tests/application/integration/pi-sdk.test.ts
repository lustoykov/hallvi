...
let root: string;
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "hallvi-sdk-regression-"));
});
afterEach(() => {
...
async function syntheticSession(compact: boolean) {
  ...
  modelRuntime.registerProvider("hallvi-sdk-test", {
    api: "hallvi-sdk-test",
  ...
  const model = modelRuntime.getModel("hallvi-sdk-test", "synthetic")!;
...
it("persists custom Run context before real auto-compaction and keeps updated outcome data in the following model request", async () => {
  ...
  try {
    await session.sendCustomMessage(
      { customType: "hallvi-run", content: latest, display: false },
      { triggerTurn: false },
    );
