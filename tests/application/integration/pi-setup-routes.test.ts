...
const startLogin = vi.hoisted(() =>
  vi.fn(() => ({ id: "attempt", state: "starting" })),
);
...
beforeEach(() => {
  ...
  vi.stubEnv(
    ...
    mkdtempSync(join(tmpdir(), "server-guy-pi-route-")),
  );
});
...
describe("Pi setup choice route", () => {
  ...
  it("passes only validated draft preferences to the login coordinator", async () => {
    ...
    const response = await LOGIN(
      new Request("http://localhost/api/pi/setup/login", {
        method: "POST",
  ...
  it.each([
    ...
    "rejects malformed login preferences before starting OAuth",
    async (body) => {
      const response = await LOGIN(
        new Request("http://localhost/api/pi/setup/login", {
          method: "POST",
  ...
  it.each([
    ...
    "rejects invalid preference requests without changing persisted configuration",
    async (body) => {
      await POST(
        new Request("http://localhost/api/pi/setup", {
          method: "POST",
  ...
  it.each([
    ...
    "rejects malformed or untrusted choice bodies without saving",
    async (body) => {
      const response = await POST(
        new Request("http://localhost/api/pi/setup", { method: "POST", body }),
      );
      expect(response.status).toBe(400);
      expect(readPiConfiguration()).toBeNull();
    },
