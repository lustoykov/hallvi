...
beforeEach(async () => {
  root = createTemporaryRoot("/tmp/server-guy-pi-workspace-");
...
it("starts one isolated container on first use and keeps a Run's sequential calls in it", async () => {
  ...
  process.env.SERVER_GUY_PROBE_TOKEN = secret;
  ...
  expect(
    ...
    ".env.example",
    ".server-guy-source.txt",
    "src/config.js",
    "src/server.js",
  ]);
  expect(
    seeded
      .find((entry) => entry.path === ".server-guy-source.txt")!
      .content.toString(),
