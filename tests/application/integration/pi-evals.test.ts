...
describe("Phase 1 eval casebook and exact graders (no model calls)", () => {
  it("has thirty-seven uniquely named cases with explicit semantic rubrics", () => {
    expect(new Set(phaseOneCases.map((c) => c.id)).size).toBe(37);
  ...
  it("selects a case subset without silently expanding invalid selections", () => {
    expect(selectPhaseOneCases().length).toBe(37);
    expect(
