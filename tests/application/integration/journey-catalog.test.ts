...
it("keeps dashboard journeys and CI scope in agreement with executable Playwright tests", () => {
  ...
  for (const test of tests) {
    ...
    expect(browserJourneys.some((item) => journeyTags[0] === `journey-${item.id}`)).toBe(true);
  }
  for (const item of browserJourneys) {
    ...
    for (const match of matches) expect(match.tags.includes("smoke"), item.id).toBe(item.smoke);
  }
});
