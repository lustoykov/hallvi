...
const common = {
  ...
  reason: "Evidence against the rubric",
};
const llm = (
...
const human = (verdict: Review["verdict"]): Review => ({
  ...common,
  type: "human",
  verdict,
...
it("keeps unjudged and old-policy answers in attention; only current passes clear", () => {
  ...
  expect(triageCase(record, [llm("pass", "phase-one-meaning-v1")]).status).toBe(
    "needs-judge",
  );
  expect(triageCase(record, [llm("pass")]).status).toBe("cleared");
  expect(triageCase(record, [llm("fail")]).status).toBe("failures");
  expect(triageCase(record, [llm("needs-discussion")]).status).toBe(
    "needs-review",
  );
});
it.each(["checks-failed", "run-error", "not-run"] as const)(
  "never clears %s, even with model and human passes",
  (outcome) => {
    expect(
      triageCase({ ...record, outcome }, [llm("pass"), human("pass")]).status,
    ).toBe("failures");
  },
);
it("checks individual failures and saved errors independently of the outcome label", () => {
  expect(
    triageCase({ ...record, checks: { noDecisions: false } }, [
      llm("pass"),
      human("pass"),
    ]).status,
  ).toBe("failures");
  expect(
    triageCase({ ...record, error: "Turn rejected" }, [llm("pass")]).status,
  ).toBe("failures");
});
it("does not clear missing automatic evidence, input or reply", () => {
  for (const patch of [{ checks: {} }, { input: null }, { reply: null }]) {
    expect(triageCase({ ...record, ...patch }, [llm("pass")]).status).toBe(
      "needs-review",
    );
  }
});
it("keeps human decisions distinct, and a later LLM pass never overwrites them", () => {
  expect(triageCase(record, [human("fail"), llm("pass")]).status).toBe(
    "failures",
  );
  expect(
    triageCase(record, [human("needs-discussion"), llm("pass")]).status,
  ).toBe("needs-review");
  expect(triageCase(record, [human("pass"), llm("fail")]).status).toBe(
    "reviewed",
  );
  expect(triageCase(record, [human("fail"), human("pass")]).label).toBe(
    "Human pass",
  );
});
it("flags a judgment made under older rubric wording without un-judging it", () => {
  expect(triageCase(record, [llm("pass")], record.rubric)).toMatchObject({
    status: "cleared",
    judged: true,
    stale: false,
  });
  expect(triageCase(record, [llm("fail")], "A newer rubric")).toMatchObject({
    status: "failures",
    label: "LLM fail",
    judged: true,
    stale: true,
  });
  expect(triageCase(record, [llm("pass")])).toMatchObject({
    judged: true,
    stale: false,
  });
  expect(
    triageCase(record, [llm("pass", "phase-one-meaning-v1")], record.rubric),
  ).toMatchObject({ status: "needs-judge", judged: false });
});
it("uses the latest judgment for the exact answer without mutating evidence", () => {
  const reviews = [
    llm("pass"),
    llm("fail"),
    { ...llm("pass"), key: "greeting:2" },
  ];
