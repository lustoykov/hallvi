...
it("renders the guide's constructs and escapes everything else", () => {
  ...
  expect(html).toContain("<table><thead><tr><th>Entry</th><th>Expected</th></tr></thead><tbody><tr><td>P1-01</td><td><strong>Pass</strong> with <code>&lt;run&gt;</code></td></tr></tbody></table>");
  expect(html).toContain("<ul><li>one</li><li>two <strong>bold</strong> continued line</li></ul>");
  expect(html).toContain("<ol><li>first</li><li>second</li></ol>");
  expect(html).toContain('<pre><code class="language-sh">npm test &lt;x&gt;</code></pre>');
...
it("wraps the rendered guide in the dashboard shell without scripts", () => {
  ...
  expect(page).toContain("<title>Acceptance &lt;guide&gt; · Server Guy Testing</title>");
  expect(page).toContain('<link rel="stylesheet" href="/dashboard.css">');
  expect(page).toContain('<a href="/guide" aria-current="page">Acceptance guide</a>');
