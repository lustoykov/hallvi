...
- **Judging:** **Judge** in the run header covers the run, labeled with what it will do (the answers the current policy has not judged, or everything again); **Judge this answer…** (or **Judge again…**) in the open answer's judgment section judges just that repetition. It opens the same one-step confirmation (count, model/effort behind Change, Start). One isolated judgment per saved answer, sequentially; failure/timeout stops later judgments and keeps earlier verdicts. No human approval or app rerun.
