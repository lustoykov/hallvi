...
- **Judging:** **Judge** in the run header covers the run, labeled with what it will do (the answers the current policy has not judged, or everything again); **Judge this answer…** (or **Judge again…**) beside "Run case again" under the open answer's title judges just that repetition. Answers you graded yourself are not counted as unjudged; judge them one at a time if you want the LLM's opinion for calibration. It opens the same one-step confirmation (count, model/effort behind Change, Start). One isolated judgment per saved answer, sequentially; failure/timeout stops later judgments and keeps earlier verdicts. No human approval or app rerun.
