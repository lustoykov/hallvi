...
Durable request coverage starts in [application/integration/pi-runs.test.ts](application/integration/pi-runs.test.ts): enqueue/claim/finish, cancellation, linked retry, bounded summaries, and real worker crash/restart with synthetic Pi. [application/unit/pi-stream.test.ts](application/unit/pi-stream.test.ts) checks reconnectable snapshot delivery. The `durable-requests` browser journey runs a disposable Next.js app **and worker**; stopping the fixture stops both and deletes its database. Live evals call the same enqueue/worker-execution code through [execute-pi-turn.ts](execute-pi-turn.ts), without a web server or separate worker process, and report Pi session counts including summarization.
...
| Eleven real agent eval cases | `SERVER_GUY_LIVE_EVALS=1 npm run eval:pi` | ChatGPT subscription via configured Pi |
