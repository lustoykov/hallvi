...
Conversation lifecycle coverage runs against the installed Pi packages with only the model scripted: `integration/pi-operator.test.ts` pins what Hallvi relies on Pi's `AgentLane` to own (identity from the moment a message is queued, order, abort, restoration after a worker goes away, the handover window), `integration/pi-worker.test.ts` runs Hallvi's real worker, tools and database (two applications, approval waits, Stop, shared servers, restart transitions, retry and compaction), `integration/pi-sessions.test.ts` covers the one-writer lock and opening an earlier history with its original kept, `integration/db-upgrade.test.ts` the database upgrade and its rollback copy, and `integration/pi-shared-account.test.ts` authentication through Pi's own `ModelRuntime`. `tests/browser/worker-restart.spec.ts` kills the real worker process mid-answer. `tests/application/unit/chat-recovery.test.tsx` covers the recovery UI. Current transcript regression coverage is `tests/browser/pi-transcript.spec.ts`; run it together with the smoke suite after transcript changes.
...
The local testing dashboard is available through `npm run test:dashboard`. It starts the application tests, the browser smoke subset and the browser journeys, and nothing it starts makes a model call or costs anything. This cleanup removes dedicated coverage of the dashboard and the browser-fixture harness because they are outside the current product acceptance work. Local tooling can still fail silently or affect data, credentials and spending: its location is not an exemption from the 80/20 bar. Check the relevant behavior when changing these tools; add a focused regression only when a concrete risk warrants it.
