...
- **Choose journeys:** opens a dialog beside the run action, without scrolling the page. Inspect descriptions and select any of eleven desktop journey groups (ten product groups, including GitHub connection, plus the dashboard). A group can contain several test cases. The shared catalog and exact test tags live in [browser/journeys.ts](browser/journeys.ts). Smoke still runs just two.
