...
const selection = {
  providerId: "openai-codex", modelId: "gpt-5.6-sol", reasoningEffort: "high" as const,
  provider: "OpenAI Codex", model: "GPT-5.6 Sol",
};
const initialStatus: PiSetupStatus = {
  state: "needs-choice", ready: false, mode: null, billing: "subscription", detected: null,
  hasSavedConfiguration: false,
  models: [{ id: selection.modelId, name: selection.model, reasoningEfforts: ["high"] }],
  separateAuthPath: "/server-guy/pi-auth.json",
  runtime: { label: "Bundled Pi SDK", detail: "No separate installation required." },
  authentication: { configured: false, label: "Not connected", source: "/server-guy/pi-auth.json" },
  selection, issue: null,
};

function render(overrides: Partial<PiSetupStatus> = {}) {
  const html = renderToStaticMarkup(<PiSetupScreen initialStatus={{ ...initialStatus, ...overrides }} />);
  // Native popover content is hidden until explicitly opened; it doesn't expand the card.
  const defaultView = html.replace(/<aside\b[^>]*popover[^>]*>[\s\S]*?<\/aside>/g, "");
...
describe("account-first setup and progressive disclosure", () => {
  it("keeps storage discoverable and explains the real protection without a security claim", () => {
    ...
    expect(html).toContain("Other software running as that same user can read them");
  ...
  it("explains both storage destinations before consent to reuse", () => {
    const { html, defaultView } = render({
      ...
      detected: {
        id: "candidate", selection, settingsPath: "/pi/settings.json", authPath: "/pi/auth.json",
        credentialType: "oauth", usesDefaultModel: false, billing: "subscription", canReuse: true, issue: null,
      },
      authentication: { configured: true, label: "Found", source: "/pi/auth.json" },
    });
    ...
    expect(html).toContain("Reuse shares Pi’s login file and copies its model preferences.");
  ...
  it("does not claim provider access was tested when only a saved login was found", () => {
    const { html, defaultView } = render({
      state: "ready", ready: true, mode: "shared", hasSavedConfiguration: true,
      authentication: { configured: true, label: "Found", source: "/approved/pi/auth.json" },
    });
    ...
    expect(html).toContain("It does not revoke OAuth tokens or sign you out of ChatGPT or Pi.");
  });

  it("offers direct sign-in without a separate storage-configuration step", () => {
    const { defaultView } = render({ state: "needs-auth", mode: "separate", hasSavedConfiguration: true });
    expect(defaultView).toContain("Connect ChatGPT");
    expect(defaultView).not.toContain("Set up a new login");
  });

  it("leaves errors and recovery visible outside the help disclosures", () => {
    const { defaultView } = render({ state: "auth-error", mode: "shared", issue: "Login file cannot be read." });
  ...
  it("shows the automatic scan result when no reusable login exists", () => {
    const { defaultView } = render({ detected: {
      id: "candidate", selection, settingsPath: "/pi/settings.json", authPath: "/pi/auth.json",
      credentialType: null, usesDefaultModel: true, billing: "subscription", canReuse: false, issue: "No credentials found.",
    } });
