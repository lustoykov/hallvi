...
it.each([false, true])("explains failed live runs without leaking output (partial report: %s)", async (partialReport) => {
  ...
  const saveReport = (run: string) => writeJson(join(directory(join(root, "tests/results/evals", run)), "results.json"), {
    model: "gpt-5.6-sol", effort: "high", startedAt: new Date().toISOString(), commit: "test", dirty: false, sourceFingerprints: {}, results: [],
  });
  ...
  const dashboard = createDashboard(root, (_command, _args, options) => {
    ...
    return spawn(process.execPath, ["-e", "console.error('FAKE-PROVIDER-SECRET');process.exit(1)"], options);
  });
  try {
    ...
    const response = await fetch(`${origin}/api/start`, { method: "POST", headers, body: JSON.stringify({
      suite: "live", consent: true, model: "gpt-5.6-sol", effort: "high", cases: ["greeting"], repeats: 1,
    }) });
    ...
    await vi.waitFor(async () => {
      ...
      expect(run.log.includes("No readable eval report was saved by this run")).toBe(!partialReport);
      expect(state.reports.map((report: { run: string }) => report.run).sort()).toEqual(partialReport ? ["older", "partial"] : ["older"]);
    });
  } finally {
    dashboard.stop();
