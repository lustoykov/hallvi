...
const browserGuide = {
  ...
  isolation: "Product journeys start one disposable Server Guy app on port 3180 with its own temporary SQLite file. All selected journeys share that file, but each creates a different application. Example: a priority saved by the isolation journey cannot appear in the revision journey. After the suite, Next.js stops; the temporary app and database remain under /tmp/server-guy-e2e-* until the operating system cleans them up. Port 3000 and your normal database are never opened. Dashboard-only journeys use synthetic dashboard state.",
...
export const suiteGuides = {
  ...
  live: {
    ...
    isolation: "Each live eval run creates one temporary eval database under /tmp/server-guy-pi-eval-*. All selected cases share that file, but every case and repetition gets a new application and chat. Example: the greeting case cannot inherit messages or Decisions from revise-existing. After the run, the database connection closes; the file remains in /tmp for debugging until the operating system cleans it up. The answer report is saved separately under tests/results/evals/. Your normal application database is never opened; configured Pi credentials are real and may be refreshed.",
