...
// A single engineer's tool: the verdict is the record; a note and a name are optional extras.
export const humanReviewSchema = z.strictObject({
  verdict: verdictSchema, reason: z.string().trim().max(5000).optional(),
  reviewer: z.string().trim().min(1).max(100).optional(),
});
export type HumanReview = z.infer<typeof humanReviewSchema>;
export function defaultReviewer() { try { return userInfo().username || "local"; } catch { return "local"; } }
...
const reviewFields = { verdict: verdictSchema, key: z.string(), sourceHash: z.string(), rubric: z.string(), createdAt: z.string() };
const savedReviewSchema = z.discriminatedUnion("type", [
  z.object({ ...reviewFields, type: z.literal("human"), reason: z.string().max(5000), reviewer: z.string().min(1).max(100) }),
  z.object({ ...reviewFields, type: z.literal("llm"), reason: z.string().min(1).max(5000), model: z.string(), effort: z.string(), promptVersion: z.string(), piVersion: z.string() }),
]);
...
export function saveReview(root: string, run: string, hash: string, key: string,
  ...
  const entry = savedReviewSchema.parse(review.type === "human"
    ? { ...base, type: "human", ...humanReviewSchema.parse({ verdict: review.verdict, reason: review.reason, reviewer: review.reviewer }), reason: review.reason?.trim() ?? "", reviewer: review.reviewer?.trim() || defaultReviewer() }
    : { ...base, ...review, ...judgmentSchema.parse({ verdict: review.verdict, reason: review.reason }) });
...
export function saveHumanReviews(root: string, run: string, hash: string, keys: string[], review: HumanReview) {
