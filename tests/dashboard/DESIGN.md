...
- LLM judgment: verdict chip, model/effort/time and the reason with its first sentence emphasized; no controls inside the block. The per-answer "Judge this answer…" / "Judge this answer again…" link sits with "Run case again…" under the answer title; the run header's Judge button is the only run-wide control and counts only queued answers without a current judgment. Judgments never fill or change the human verdict.
