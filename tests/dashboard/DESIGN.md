...
- LLM judgment: verdict chip, an amber "Older rubric wording" chip when the casebook wording has changed since, model/effort/time, the reason with its first sentence emphasized, and one small secondary button at the right, "Judge this answer…" or "Judge again…", for this repetition. The run header's Judge button is the only run-wide control and counts only queued answers without a current judgment. Judgments never fill or change the human verdict.
