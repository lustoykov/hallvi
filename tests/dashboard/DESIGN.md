...
- Pickers: suite scope selection uses native dialogs with one-column checklists, a scrollable body and a visible footer; closing keeps selections. Eval cases have searchable, collapsible categories; a tri-state checkbox beside each category name selects that group's visible cases without expanding it, and rows carry compact names and run-history labels; expected behavior and input sit behind a disclosure. Only cases without a saved attempt are selected by default, including archived history; skipped or merely planned cases remain unrun. Polling updates this default until the user makes a manual selection; Select unrun restores it. Categories containing unrun cases start expanded. Eval repetitions default to one, with 2–5 available explicitly.
