...
- Recent runs: the latest eight rows, then "Show all N runs". A judge row names the run it judged, and an open output panel stays visible even when its row is older than the fold.
