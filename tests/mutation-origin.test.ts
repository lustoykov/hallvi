...
const effects = vi.hoisted(() => ({
  ...
  createChat: vi.fn(() => ({})), sendChatMessage: vi.fn(() => ({})),
  archiveChat: vi.fn(() => ({})), observeRepository: vi.fn(() => ({})),
...
const routes = [
  { name: "application", method: "POST", handler: application, body: { repositoryUrl: "https://github.com/qa/example", approvalMode: "pi-decides" } },
  { name: "chat", method: "POST", handler: chat, body: {} },
  { name: "message", method: "POST", handler: message, body: { message: "Hello" } },
  ...
  { name: "disconnect", method: "DELETE", handler: disconnect, body: { confirm: "disconnect" } },
  { name: "remove", method: "DELETE", handler: remove, body: { repository: "qa/example" } },
];

beforeEach(() => vi.clearAllMocks());

describe.each(routes)("$name mutation Origin boundary", ({ method, handler, body }) => {
  it.each(["https://untrusted.example", "null", "http://127.0.0.1:3999"])("rejects %s before any side effect", async (origin) => {
    const request = new NextRequest("http://localhost:3000/api/test", {
      method, body: body === undefined ? undefined : JSON.stringify(body),
      headers: { origin, host: "127.0.0.1:3000", "content-type": "text/plain", "x-forwarded-host": "untrusted.example" },
    });
  ...
  it.each(["http://127.0.0.1:3000", undefined])("accepts matching Origin or an Origin-less local client: %s", async (origin) => {
    const request = new NextRequest("http://localhost:3000/api/test", {
      method, body: body === undefined ? undefined : JSON.stringify(body),
      headers: { ...(origin ? { origin } : {}), host: "127.0.0.1:3000", "content-type": "application/json" },
    });
    const response = await handler(request, context);
    expect(response.status).toBeLessThan(300);
    expect(Object.values(effects).some((effect) => effect.mock.calls.length)).toBe(true);
  });
});
