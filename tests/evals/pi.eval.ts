...
import type { PiTurnResult } from "../../src/server/types";
import { pushTestDatabase } from "../test-database";
import { checkTurn } from "./checks";
import { evalRepeatCount, selectEvalCases } from "./cases";
import { createEvalScratch, releaseEvalScratch } from "./scratch";
import { evalView, seedEvalCase, type EvalView } from "./seed";
...
const selectedCases = selectEvalCases(process.env.PI_EVAL_CASES);
const sourceFiles = [
  "src/server/pi.ts",
...
beforeAll(() => {
  ...
  // Every GitHub read fails here; the fixture token never reaches a network.
  ...
  // Repository archives are blocked the same way.
...
for (let repetition = 1; repetition <= repeats; repetition++) {
  for (const scenario of selectedCases) {
    it(`${scenario.id} / repetition ${repetition}`, async (context) => {
      let before = seedEvalCase(scenario, repetition);
      ...
      try {
        ...
        record.after = evalView(before.application!.id, before.selectedChatId!);
        record.input = turn.mock.calls[0]?.[0] ?? null;
        record.reply = await turn.mock.results[0].value;
        record.checks = checkTurn(
          scenario,
          before,
          record.after,
          record.reply!,
