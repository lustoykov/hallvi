...
test("P1-09 confirmed removal permits a genuinely fresh application", journey("removal"), async ({ page }) => {
  ...
  await expect(dialog.getByRole("button", { name: "Remove application", exact: true })).toBeDisabled();
  await dialog.getByRole("textbox").fill("qa/removal-app");
  await dialog.getByRole("button", { name: "Remove application", exact: true }).click();
...
test("P1-10 disconnect preserves application history and requires consent to reuse", journey("disconnect"), async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "View applications" })).toBeDisabled();
