...
test(
  ...
  async ({ page }, testInfo) => {
    ...
    const dashboard = createDashboard(root, () => {
      launches++;
      throw new Error("No runner allowed in this fixture");
    });
    for (const entry of [
      {
      ...
      writeJson(join(root, "tests/results/runs", `${entry.id}.json`), {
        ...
        finishedAt: "2026-09-04T09:00:02Z",
        status: "passed",
        log: "Fixture output",
        commit: "test",
        dirty: false,
    ...
    writeJson(
      join(directory(join(root, "tests/results/evals", run)), "results.json"),
      {
        ...
        results: [
          {
            caseId: "question",
            repetition: 1,
            rubric: "Discuss without pretending a choice was made",
            outcome: "checks-passed",
            checks: { count: true },
            error: null,
            input: { userMessage: "Should I prioritize simplicity?" },
            reply: {
              message:
                "<script>window.bad=true</script> Let’s discuss the tradeoff.",
              decisionProposals: [],
            },
            before: {},
            after: {},
          },
          {
            caseId: "greeting",
            repetition: 1,
            rubric: "Greet without a Decision",
            outcome: "checks-passed",
            checks: { count: true },
            error: null,
        ...
        ].flatMap((record) => [record, { ...record, repetition: 2 }]),
      },
    );
    ...
    // Verify requested selections/consent without launching real tests or any
    // model.
    ...
    const answer = (key: string) =>
      page.locator(`.answer-open[data-case-key="${key}"]`);
    const runCard = (id: string) => page.locator(`.run-card[data-run="${id}"]`);
    const verdict = (name: string) =>
      page.locator("#verdict").getByRole("button", { name, exact: true });
    const filterCount = (filter: string) =>
      page.locator(`.filters [data-filter="${filter}"] span`);
    const startButton = page.getByRole("button", {
      name: /^Start (run|judging)$/,
    });
    try {
      await page.goto(`http://127.0.0.1:${address.port}`);
      await expect(page.getByRole("row")).toHaveCount(5);
      await expect(
        page.getByRole("columnheader", { name: "AI usage", exact: true }),
      ).toBeVisible();
      await expect(
        page.getByRole("columnheader", { name: "CI", exact: true }),
      ).toBeVisible();
      ...
      const liveRow = page
        .getByRole("row")
        .filter({ hasText: "Live agent evals" });
      await expect(liveRow.locator(".usage-paid")).toHaveText(
        "Uses subscription",
      );
      await expect(liveRow.locator(".suite-schedule > span")).toHaveText(
        "On demand only",
      );
      await expect(liveRow.locator(".suite-schedule small")).toHaveText(
        "Local only · never in CI",
      );
      await expect(
        liveRow.getByRole("link", { name: /View saved runs/ }),
      ...
      await expect(
        page.getByText("Full suite: manual", { exact: true }),
      ).toHaveCount(0);
      // Reference material lives on its own page; the suite table stays a plain
      // table of actions.
      await expect(
      ...
      await expect(page.locator("#about-panel")).toContainText(
        "The tests still run automatically after you start them.",
      );
      await expect(page.locator("#about-panel")).toContainText(
        "GitHub-hosted runs still consume Actions minutes.",
      );
      await expect(page.locator("#compare thead th")).toHaveText([
        "",
        "Application tests",
        "Browser smoke",
        "Browser journeys",
      ...
      // Inner result stacks must not turn table cells into grids or break row
      // borders.
      for (const row of await page.locator("#suites tr").all()) {
        const cells = row.locator("td");
        await expect(cells).toHaveCount(5);
        const bounds = await cells.evaluateAll((items) =>
          items.map((cell) => {
        ...
        expect(bounds.every((cell) => cell.display === "table-cell")).toBe(
          true,
        );
        expect(
          Math.max(...bounds.map((cell) => cell.bottom)) -
            Math.min(...bounds.map((cell) => cell.bottom)),
        ).toBeLessThan(1);
      }
      const actionBounds = await page
        .locator("#suites td:last-child button")
        .evaluateAll((buttons) =>
          buttons.map((button) => {
      ...
      for (const dimension of ["left", "width", "height"] as const) {
        expect(
          Math.max(...actionBounds.map((box) => box[dimension])) -
            Math.min(...actionBounds.map((box) => box[dimension])),
        ).toBeLessThan(1);
      }
      await page
        .getByRole("button", { name: "Browser smoke", exact: true })
        .click();
      await expect(page.locator("#log")).toHaveText(
        "Command not recorded for this older run.\n\nFixture output",
      );
      await page
        .getByRole("button", { name: "Application tests", exact: true })
        .click();
      await expect(page.locator("#log")).toHaveText(
        "$ npm run test\n\nFixture output",
      );
      await page.screenshot({
        path: testInfo.outputPath("dashboard-runs.png"),
        fullPage: true,
      });
      await page.getByRole("button", { name: "Choose journeys…" }).click();
      await expect(
        page.getByRole("dialog", { name: "Choose browser journeys" }),
      ).toBeVisible();
      await expect(page.locator("#journey-options input")).toHaveCount(
        browserJourneys.length,
      );
      ...
      await page
        .getByRole("checkbox", { name: /^Settings and privacy help/ })
        .check();
      await page
        .getByRole("checkbox", { name: /^Disconnect without losing history/ })
        .check();
      await expect(page.locator("#journey-count")).toContainText(
        `2 of ${browserJourneys.length} journeys selected`,
      );
      await page.screenshot({
        path: testInfo.outputPath("dashboard-journey-selection.png"),
        fullPage: true,
      });
      await page.getByRole("dialog").press("Escape");
      await expect(
        page.getByRole("button", { name: "Choose journeys…" }),
      ).toBeFocused();
      await page.getByRole("button", { name: "Choose journeys…" }).click();
      await expect(page.locator("#journey-count")).toContainText(
        `2 of ${browserJourneys.length} journeys selected`,
      ...
      await expect
        .poll(() => requests.at(-1))
        .toEqual({ suite: "e2e", journeys: ["settings", "disconnect"] });
      await expect(page.getByRole("dialog")).toHaveCount(0);
      // A live run is one confirmation: settings shown, judging afterwards on
      // by default.
      ...
      await expect(page.locator("#eval-count")).toHaveText(
        `${evalCases.length - 1} cases × 1 repetition = ${evalCases.length - 1} planned answers`,
      );
      await page
        .getByRole("button", { name: "Clear cases", exact: true })
        .click();
      await expect(
        page.getByRole("button", { name: "Review and run live evals…" }),
      ).toBeDisabled();
      await page.getByRole("checkbox", { name: /^Ordinary greeting/ }).check();
      await page
        .getByRole("checkbox", { name: /^Hypothetical choice/ })
        .check();
      await page.getByLabel("Repetitions per case").selectOption("2");
      await expect(page.locator("#eval-count")).toHaveText(
        "2 cases × 2 repetitions = 4 planned answers",
      );
      await page.screenshot({
        path: testInfo.outputPath("dashboard-eval-selection.png"),
        fullPage: true,
      });
      await page
        .getByRole("button", { name: "Review and run live evals…" })
        .click();
      await expect(
        page.getByRole("dialog", { name: "Run 2 cases", exact: true }),
      ).toBeVisible();
      await expect(page.locator("#confirm-description")).toContainText(
        "2 cases × 2 repetitions = 4 planned answers",
      );
      ...
      // Review: the newest run opens with its first answer needing attention;
      // repetitions of a case sit together.
      ...
      // A direct link in a fresh tab and a reload both resolve server-side to
      // the review page.
      const direct = await page.context().newPage();
      await direct.goto(`http://127.0.0.1:${address.port}/evals`);
      await expect(
        direct.getByRole("heading", { name: "Eval runs", exact: true }),
      ).toBeVisible();
      await direct.reload();
      await expect(direct.locator("#case-title")).toContainText("question");
      await direct.close();
      await expect(page.locator("#run-meta")).toContainText(
        "2 cases × 2 repetitions = 4 planned · 4 answers saved",
      );
      await expect(page.locator("#progress-text")).toHaveText(
        "4 need attention · 0 of 4 human-reviewed",
      );
      // Judging a run defaults to its unjudged answers, with one step to start.
      await page
        .getByRole("button", { name: "Judge 4 unjudged answers…", exact: true })
        .click();
      await expect(
        page.getByRole("dialog", {
          name: "Judge 4 unjudged answers",
          exact: true,
        }),
      ).toBeVisible();
      await expect(page.locator("#confirm-selection")).toHaveText(
        "question:1, question:2, greeting:1, greeting:2",
      );
      await expect(page.locator("#confirm-description")).toContainText(
        "Server Guy is not rerun",
      );
      await expect(page.locator("#judge-after-row")).toBeHidden();
      await expect(
        page.getByRole("button", { name: "Start judging", exact: true }),
      ).toBeEnabled();
      await page.getByRole("button", { name: "Cancel", exact: true }).click();
      await expect(
      ...
      await expect(page.locator("#answer-list .answer-group")).toHaveText([
        "question",
        "Ordinary greeting",
      ]);
      await expect(page.locator("#answer-list .answer-open")).toHaveText([
        /^Repetition 1/,
        /^Repetition 2/,
        /^Repetition 1/,
        /^Repetition 2/,
      ]);
      await expect(page.locator("#run-repeats")).toContainText(
        "answered 2 times",
      );
      await expect(answer("question:1")).toHaveAttribute(
        "aria-current",
        "true",
      );
      ...
      // Rerun one current case once, even when the picker and saved run use two
      // repetitions.
      // Old/removed case IDs remain readable but cannot start a different case
      // accidentally.
      await expect(
        page.getByRole("button", { name: "Run case again…", exact: true }),
      ).toBeHidden();
      await expect(page.locator("#rerun-unavailable")).toBeVisible();
      await answer("greeting:1").click();
      const beforeRerun = requests.length;
      await page
        .getByRole("button", { name: "Run case again…", exact: true })
        .click();
      await expect(
        page.getByRole("dialog", {
          name: "Run Ordinary greeting",
          exact: true,
        }),
      ).toBeVisible();
      await expect(page.locator("#confirm-description")).toContainText(
        "1 case × 1 repetition = 1 planned answer",
      );
      await expect(page.locator("#confirm-description")).toContainText(
        "existing answers and reviews stay unchanged",
      );
      await expect(page.locator("#confirm-selection")).toHaveText("greeting");
      await page.screenshot({
        path: testInfo.outputPath("dashboard-single-case-rerun.png"),
        fullPage: true,
      });
      await page.getByRole("button", { name: "Cancel", exact: true }).click();
      ...
      // One click is a verdict; a note is optional and survives switching pages
      // until it is saved.
      ...
      await page
        ...
        .fill("Please clarify the intended behavior.");
      ...
      await page.screenshot({
        path: testInfo.outputPath("dashboard-verdict.png"),
        fullPage: true,
      });
      // LLM advice is displayed beside the saved human verdict without changing
      // it.
      saveReview(root, run, hash, "question:1", {
        type: "llm",
        ...
        verdict: "pass",
        reason:
          "The reply discusses the tradeoff without recording an invented choice. Nothing in it claims a decision was made.",
      });
      await page.reload();
      await expect(page).toHaveURL(`http://127.0.0.1:${address.port}/evals`);
      await expect(verdict("Discuss")).toHaveAttribute("aria-pressed", "true");
      const advice = page.getByRole("region", {
        name: "LLM judgment",
        exact: true,
      });
      await expect(advice.locator(".lead")).toHaveText(
        "The reply discusses the tradeoff without recording an invented choice.",
      );
      await expect(advice.locator(".judgment-more")).not.toHaveAttribute(
        "open",
        "",
      );
      await advice.getByText("Full reasoning", { exact: true }).click();
      await expect(advice.locator(".judgment-more")).toContainText(
        "Nothing in it claims a decision was made.",
      );
      await expect(page.locator("#judge-verdict")).toHaveText("Pass");
      await expect(page.locator("#triage-chip")).toHaveText("Needs review");
      await expect(
        page.locator("#verdict .verdict-button.pass .llm-tag"),
      ).toBeVisible();
      await expect(answer("question:1")).toContainText("LLM advice: Pass");
      await page.screenshot({
        path: testInfo.outputPath("dashboard-review.png"),
        fullPage: true,
      });
      // One answer can be judged on its own from its judgment section.
      await advice
        .getByRole("button", { name: "Judge again…", exact: true })
        .click();
      await expect(
        page.getByRole("dialog", { name: "Judge this answer", exact: true }),
      ).toBeVisible();
      await expect(page.locator("#confirm-selection")).toHaveText("question:1");
      await page
        .getByRole("button", { name: "Start judging", exact: true })
        .click();
      await expect
        .poll(() => requests.at(-1))
        .toMatchObject({
      ...
      for (const remaining of [2, 1, 0]) {
        await page.keyboard.press("p");
        await expect(page.locator("#answer-list .answer-open")).toHaveCount(
          remaining,
        );
      }
      await expect(page.locator("#no-answers")).toContainText(
        "Nothing needs attention",
      );
      await expect(page.locator("#progress-text")).toHaveText(
        "Nothing needs attention · 4 of 4 human-reviewed",
      );
      await expect(reviewTab).not.toContainText("need attention");
      // With nothing left in the queue, the run-level button means "judge
      // everything again" and steps back to secondary.
      await expect(page.locator("#judge-run")).toHaveClass("secondary");
      await page
        .getByRole("button", { name: "Judge run again…", exact: true })
        .click();
      await expect(
        page.getByRole("dialog", {
          name: "Judge all 4 answers again",
          exact: true,
        }),
      ).toBeVisible();
      ...
      await expect
        ...
        .toMatchObject({
          suite: "judge",
          keys: ["question:1", "question:2", "greeting:1", "greeting:2"],
          consent: true,
          hash,
          model: "gpt-5.6-luna",
        });
      ...
      await page.screenshot({
        path: testInfo.outputPath("dashboard-all-reviewed.png"),
        fullPage: true,
      });
      // Archive moves the whole run, keeps it readable, and a failed request
      // keeps the selection.
      await page.route(
        "**/api/runs/archive",
        (route) =>
          route.fulfill({
            status: 400,
            json: { error: "Results changed. Reload before archiving." },
          }),
        { times: 1 },
      );
      await page
      ...
      await page
        ...
        .click();
      await expect(
        page.locator("#archived-list").locator(runCard(run)),
      ).toBeVisible();
      ...
      await page
        .getByRole("button", { name: "Restore run", exact: true })
        .click();
      await expect(
      ...
      expect(
        savedReviews.find((r) => r.type === "human" && r.key === "question:1"),
      ).toMatchObject({
        verdict: "needs-discussion",
        reason: "Please clarify the intended behavior.",
      });
      expect(
        savedReviews
          .filter((r) => r.type === "human")
          .every((r) => r.reviewer.length > 0),
      ).toBe(true);
      // An older run with a failed answer: selection stays inside one run,
      // failed answers can't be selected.
      const secondRun = "other-synthetic-run";
      const saved = loadReport(root, run).report;
      writeJson(
        join(
          directory(join(root, "tests/results/evals", secondRun)),
          "results.json",
        ),
        {
          ...saved,
          ...
          results: saved.results
            .filter((c) => c.caseId === "greeting")
            .map((c) =>
              c.repetition === 1
                ? c
                : { ...c, input: null, reply: null, outcome: "not-run" },
            ),
        },
      );
      await expect(page.locator("#run-list .run-card")).toHaveCount(2);
      await page.screenshot({
        path: testInfo.outputPath("dashboard-two-runs.png"),
        fullPage: true,
      });
      await runCard(secondRun).click();
      await expect(
        page.getByRole("button", {
          name: "Judge 1 unjudged answer…",
          exact: true,
        }),
      ).toBeEnabled();
      ...
      await expect(
        page.getByRole("button", { name: "Judge this answer…", exact: true }),
      ).toBeDisabled();
      await expect(
        page.getByRole("button", { name: "Run case again…", exact: true }),
      ).toBeEnabled();
      await page
        ...
        .click();
      await expect(
        page.locator("#archived-list").locator(runCard(secondRun)),
      ).toBeVisible();
      await runCard(run).click();
      await expect(filterCount("all")).toHaveText("4");
      await expect(reviewTab).not.toContainText("need attention");
      expect(listReports(root).find((r) => r.run === secondRun)?.archived).toBe(
        true,
      );
      ...
      const caseIds = Array.from(
        { length: 8 },
        (_, index) => `layout-case-${index + 1}`,
      );
      writeJson(
        join(
          directory(join(root, "tests/results/evals", fullRun)),
          "results.json",
        ),
        {
          ...saved,
          ...
          repeats: 2,
          plannedCases: 16,
          results: caseIds.flatMap((caseId) =>
            [1, 2].map((repetition) => ({
              ...saved.results[0],
              caseId,
              repetition,
      ...
      await page.getByRole("link", { name: "Run checks", exact: true }).click();
      await reviewTab.click();
      await expect(answer("layout-case-8:2")).toHaveAttribute(
        "aria-current",
        "true",
      );
      // Long titles and metadata keep their navigation controls at smaller
      // desktop widths.
      ...
      writeJson(
        ...
        {
          ...fullReport,
          caseIds: [longCaseId, ...caseIds.slice(1)],
          results: fullReport.results.map((record) =>
            record.caseId === caseIds[0]
              ? { ...record, caseId: longCaseId }
              : record,
          ),
        },
      );
      ...
      const navigationBox = await page
        .getByRole("navigation", { name: "Answer navigation" })
        .boundingBox();
      ...
      await page.screenshot({
        path: testInfo.outputPath("dashboard-desktop-long-title.png"),
        fullPage: true,
      });

      // Judge-first triage: current passes clear; failures and uncertainty stay
      // visible.
      ...
      writeJson(
        join(
          directory(join(root, "tests/results/evals", triageRun)),
          "results.json",
        ),
        {
          ...saved,
          ...
          caseIds: triageIds,
          repeats: 1,
          plannedCases: triageIds.length,
          results: triageIds.map((caseId) => ({
            ...saved.results[0],
            caseId,
            repetition: 1,
            checks: { count: caseId !== "blocked" },
          })),
        },
      );
      const triageHash = loadReport(root, triageRun).hash;
      for (const [caseId, verdictValue] of [
        ["clear", "pass"],
        ["fail", "fail"],
        ...
        saveReview(root, triageRun, triageHash, `${caseId}:1`, {
          type: "llm",
          model: "synthetic-judge",
          effort: "high",
          promptVersion:
            caseId === "legacy" ? "phase-one-meaning-v1" : JUDGE_PROMPT_VERSION,
          piVersion: "fixture",
          verdict: verdictValue,
          reason: `Saved evidence for ${caseId}.`,
        });
      }
      saveReview(root, triageRun, triageHash, "human:1", {
        type: "human",
        reviewer: "Fixture reviewer",
        verdict: "pass",
        reason: "Human assessment stays separate.",
      });
      ...
      await expect(page.locator("#progress-text")).toHaveText(
        "5 need attention · 2 LLM-cleared · 1 of 8 human-reviewed · judge agreed 0 of 1",
      );
      await page.getByRole("button", { name: /^Reviewed/ }).click();
      await answer("human:1").click();
      await expect(page.locator("#triage-reason")).toHaveText(
        "The judge said fail; your verdict wins.",
      );
      await page.getByRole("button", { name: /^Attention/ }).click();
      await expect(
        page.getByRole("button", { name: /^Attention/ }),
      ...
      await expect(page.locator("#triage-reason")).toContainText(
        "Older judge policy",
      );
      ...
      await expect(page.locator("#answer-list .answer-open")).toHaveCount(2);
      // A judgment saved under older rubric wording keeps its verdict but is
      // flagged, and the current wording is shown.
      await answer("greeting:1").click();
      ...
      await expect(page.locator("#rubric-saved")).toContainText(
        "Discuss without pretending a choice was made",
      );
      ...
      expect(
        listReports(root)
          .find((report) => report.run === triageRun)
          ?.reviews.filter((review) => review.type === "human"),
      ).toHaveLength(1);
      await page.screenshot({
        path: testInfo.outputPath("dashboard-judge-triage.png"),
        fullPage: true,
      });
      // Judging the run targets only what the current policy has not judged,
      // regardless of the display filter.
      await page
        .getByRole("button", { name: "Judge 2 unjudged answers…", exact: true })
        .click();
      await expect(page.locator("#confirm-selection")).toHaveText(
        "legacy:1, unjudged:1",
      );
      ...
      await expect
        ...
        .toMatchObject({
          ...
          run: triageRun,
          hash: triageHash,
          keys: ["legacy:1", "unjudged:1"],
          consent: true,
        });
      // A human can reject a spot-checked pass; the answer returns to Failed,
      // not hidden clearance, and the queue moves on.
      ...
      // With nothing unjudged, the Judge button re-judges what the current
      // filter shows, or the whole run under All.
      const judgedRun = "judged-run";
      writeJson(
        join(
          directory(join(root, "tests/results/evals", judgedRun)),
          "results.json",
        ),
        {
          ...saved,
      ...
      for (const [caseId, verdictValue] of [
        ...
        saveReview(root, judgedRun, judgedHash, `${caseId}:1`, {
          type: "llm",
          model: "synthetic-judge",
          effort: "high",
      ...
      await expect(page.locator("#judge-run")).toHaveClass("secondary");
      await page
        .getByRole("button", { name: "Judge this one again…", exact: true })
        .click();
      await expect(
        page.getByRole("dialog", { name: "Judge 1 answer again", exact: true }),
      ).toBeVisible();
      ...
      await expect(
        page.getByRole("dialog", {
          name: "Judge all 3 answers again",
          exact: true,
        }),
      ).toBeVisible();
      await expect(page.locator("#confirm-selection")).toHaveText(
