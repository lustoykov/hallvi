...
test("live eval settings are read-only while a saved-answer judge can choose its model", journey("dashboard"), async ({ page }, testInfo) => {
  const root = mkdtempSync(join(tmpdir(), "sg-confirmation-e2e-"));
  let launches = 0;
  const dashboard = createDashboard(root, () => { launches++; throw new Error("No model calls allowed"); });
  ...
  writeJson(join(directory(join(root, "tests/results/evals", "saved")), "results.json"), {
    model: "synthetic", effort: "high", startedAt: "2026-09-04", commit: "test", dirty: false, sourceFingerprints: {},
    results: [{ caseId: "greeting", repetition: 1, rubric: "Greet without inventing a choice", outcome: "checks-passed", checks: { count: true },
  ...
  try {
    ...
    await expect(page.locator("#live-settings")).toHaveAttribute("href", "http://127.0.0.1:3000/setup/pi");
    await expect(page.locator("#run-model")).toHaveJSProperty("readOnly", true);
    ...
    await page.screenshot({ path: testInfo.outputPath("live-confirmation.png") });
    if (await page.locator("#consent").isVisible()) await page.locator("#consent").check();
    await page.locator("#confirm-run").click();
    await expect.poll(() => requests[0]).toMatchObject({ suite: "live", model, effort, cases: ["greeting"], repeats: 1, consent: true });
    await page.locator("#judge-run").click();
    await expect(page.locator("#live-settings")).toBeHidden();
    if (await page.locator("#change-settings").isVisible()) await page.locator("#change-settings").click();
    ...
    if (await page.locator("#consent").isVisible()) await page.locator("#consent").check();
    await page.locator("#confirm-run").click();
    await expect.poll(() => requests[1]).toMatchObject({ suite: "judge", model: "gpt-5.6-luna", effort: "low", consent: true });
