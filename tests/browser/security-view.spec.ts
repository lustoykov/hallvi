...
test("Security reads firewall facts, retains evidence on errors and retries", async ({
  page,
}) => {
  ...
  await page.route("**/api/applications/*/deployment", (route) =>
    route.fulfill({
      json: {
        ...
        deployment: {
          ...record,
          applicationId: new URL(route.request().url()).pathname.split("/")[3],
        },
      },
    }),
  );
  ...
  await nav
    .getByRole("button", { name: "Security nothing recorded yet", exact: true })
    .click();
  ...
  await expect(panel).toContainText("The last successful check is shown below");
  await expect(panel).toContainText("192.0.2.4/32");
  fail = false;
  ...
  await expect(
    page.getByRole("heading", { name: "Reported incoming rules", exact: true }),
  ).toHaveCount(0);
});
