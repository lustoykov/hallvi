...
test("Record tabs support keyboard navigation and long Decisions fit the desktop inspector", journey("chat-navigation"), async ({ page }, testInfo) => {
  ...
  const later = page.locator("details").filter({ hasText: "Later phases (5–6)" });
  await expect(later).not.toHaveAttribute("open", "");
  await expect(later.getByText("Hetzner access", { exact: true })).not.toBeVisible();
  await later.locator("summary").focus();
  await page.keyboard.press("Enter");
  await expect(later.getByText("Planning reminders only. These do not block Phase 1.", { exact: true })).toBeVisible();
  await expect(later.locator("article").filter({ hasText: "Hetzner access" })).toContainText("Phase 5");
  await expect(later.locator("article").filter({ hasText: "Cloudflare access" })).toContainText("Phase 6");
  await expect(later.locator("article").filter({ hasText: "Domain starting state" })).toContainText("Phase 6");
  await expect(later.getByText("missing", { exact: true })).toHaveCount(0);
  await page.screenshot({ path: testInfo.outputPath("later-phase-reminders.png") });
