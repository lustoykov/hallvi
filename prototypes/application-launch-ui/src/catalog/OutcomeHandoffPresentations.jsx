...
export function OutcomePresentation({ onOpenInspector = noop, onRecord = noop, selectedChoice = "" }) {
  ...
  return (
    <section className="presentation outcome-presentation" aria-labelledby="outcome-title">
      <header>
        <span className="mini-dot success" aria-hidden="true" />
        <strong id="outcome-title">L8.5 · Production release verified</strong>
        <span className="status-badge success">Verified live</span>
      </header>

      <div className="presentation-body">
        <div className="metric-grid" aria-label="Release identity">
          <div className="metric">
            <small>Production URL</small>
            <strong>https://northstar.dev</strong>
          </div>
          <div className="metric">
            <small>Release SHA</small>
            <strong><code>9f3c2ad</code></strong>
          </div>
          <div className="metric">
            <small>Verified at</small>
            <strong>10:42:18 UTC</strong>
          </div>
        </div>
        ...
        <div className="check-list" aria-label="Release verification results">
          {verificationResults.map(([label, result, evidence]) => (
            <div className="check-row" key={label}>
              <span className="mini-dot success" aria-hidden="true" />
              <span><strong>{label}</strong> <small>{evidence}</small></span>
              <span className="status-badge success">{result}</span>
        ...
        <div className="callout warning">
          <strong>Accepted gap</strong> · A product-specific business metric is not defined yet. Core health, errors, logs, and uptime remain observable. <button type="button" onClick={() => onOpenInspector("record")}>Review decision</button>
        </div>
        ...
        <div className="presentation-actions">
          <button
            ...
            type="button"
            onClick={() => onRecord("Opened the production application at https://northstar.dev.")}
          >
            Open application
          </button>
...
export function HandoffPresentation({ onOpenInspector = noop, onRecord = noop, selectedChoice = "" }) {
  ...
  return (
    <section className="presentation handoff-presentation" aria-labelledby="handoff-title">
      <header>
        <span className="mini-dot active" aria-hidden="true" />
        <strong id="handoff-title">Pull Request #42 · health endpoint</strong>
        <span className="status-badge warning">Awaiting review</span>
      </header>
      ...
      <div className="presentation-body">
        <dl>
          <div className="field-row">
            <dt>Branch</dt>
            <dd><code>feature/health-endpoint</code> → <code>main</code></dd>
          </div>
          <div className="field-row">
            <dt>Commit</dt>
            <dd><code>9f3c2ad4b1e8c8f7a5e2</code></dd>
          </div>
        </dl>
          ...
          <div className="file-list">
            {changedFiles.map(([path, delta, summary]) => (
              <div className="file-row" key={path}>
                <span className="mini-dot success" aria-hidden="true" />
                <span><strong><code>{path}</code></strong> <small>{summary}</small></span>
                <strong>{delta}</strong>
                <button type="button" onClick={() => onOpenInspector("changes")}>Diff</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="check-list">
            {ciChecks.map(([name, status, duration]) => (
              <div className="check-row" key={name}>
                <span className="mini-dot success" aria-hidden="true" />
                <strong>{name}</strong>
                <small>{status} · {duration}</small>
                <button type="button" onClick={() => onOpenInspector("evidence")}>Evidence</button>
              </div>
            ))}
            <div className="check-row">
              <span className="mini-dot active" aria-hidden="true" />
              <strong>Smoke / profile check</strong>
              <small>Running · required before merge</small>
              <button type="button" onClick={() => onOpenInspector("evidence")}>Evidence</button>
            </div>
          </div>
        </div>
        ...
        <div className="callout">
          Pi reviewed the worker result for scope and repository conformance. The remaining smoke/profile check requires the launch environment.
        </div>
        ...
        <div className="presentation-actions">
          ...
          <button
            type="button"
            onClick={() => onRecord("Merge Pull Request #42 when the smoke/profile check passes.")}
          >
            Merge when green
          </button>
