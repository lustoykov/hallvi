    ...
    <main className="chat-panel" aria-label="Chat with Pi">
      ...
      <div className="chat-scroll">
        ...
        <section className="activity-block"><div className="activity-label"><span>Live activity toward {phase.deliverable}</span><small>Evidence-backed</small></div>{current.events.map((event) => {
          ...
          return <button className="activity-event" key={`${title}-${detail}`} type="button" onClick={() => onReviewEvidence(event, `${phase.deliverable} activity`)}><span className={`event-marker ${statusClass(status)}`} /><span><strong>{title}</strong><small>{detail}</small></span><em className={statusClass(status)}>Details · {status}</em></button>;
        })}</section>
...
function RecordRow({ label, value, meta, onReview }) {
  return <div className="record-row"><div><span>{label}</span><strong>{value}</strong></div><div className="record-row-actions"><em>{meta}</em>{onReview && <button type="button" className="control-link" onClick={onReview}>Details</button>}</div></div>;
}

function PhaseContract({ phase, gateProgress, current, onReviewDeliverable, onReviewGate }) {
  return (
    <section className="phase-contract-card">
      <div className="deliverable-heading"><div><span>Phase deliverable</span><strong>{phase.deliverable}</strong></div><div className="deliverable-actions"><button type="button" className="control-link" onClick={onReviewDeliverable}>Details</button><em>{gateProgress}/{phase.gate.length} passed</em></div></div>
      <p>{phase.outcome}</p>
      ...
      <div className="gate-list">{phase.gate.map((check, checkIndex) => {
        ...
        return <div key={check} className={`gate-item ${passed ? "passed" : ""}`}><span>{passed ? "Passed" : currentBlocker ? "Current" : "Required"}</span><strong>{check}</strong><button type="button" className="control-link" onClick={() => onReviewGate({ check, checkIndex, passed, currentBlocker })}>Details</button><em className={statusClass(markerState)}>{passed ? "pass" : currentBlocker ? "now" : "next"}</em></div>;
      })}</div>
    </section>
  );
}
  ...
  return (
    <aside className="operator-record" aria-label="Operator Record">
      ...
      <div className="record-scroll">
        ...
        {current.decision && <div className="decision-banner"><div><strong>Workshop choice unresolved</strong><button type="button" className="control-link" onClick={() => onReviewDecision({ label: "Workshop choice", value: current.decision, meta: "Product workshop" })}>Details</button></div><p>{current.decision}</p></div>}
...
function EvidenceRow({ event, parentTitle, onReviewEvidence }) {
  ...
  return (
    <div className="drawer-event">
      ...
      <div className="drawer-event-actions"><button type="button" className="control-link" onClick={() => onReviewEvidence(event, parentTitle)}>Details</button><span className={`mini-status ${statusClass(status)}`}>{status}</span></div>
    </div>
  );
}
...
function ControlDrawer({ control, onClose, onAskPi, onReviewEvidence }) {
  ...
  return (
    <div className="drawer-backdrop" onMouseDown={onClose} role="presentation">
      <aside className="evidence-drawer control-drawer" onMouseDown={(event) => event.stopPropagation()} aria-label={`Details for ${control.title}`}>
        ...
        <section className="control-path-section"><span className="drawer-label">Ask Pi</span><button type="button" className="control-path primary-path" onClick={() => onAskPi(control)}><strong>Ask Pi</strong><small>Get a contextual explanation, investigation, or proposed next action. Pi's answer is not proof or the governing rule.</small></button></section>
        <section><span className="drawer-label">Read contract or source</span><button type="button" className="source-link" onClick={() => setSourceOpen((value) => !value)}>{control.source}<span>{sourceOpen ? "Hide" : "Read"}</span></button>{sourceOpen && <div className="source-detail"><strong>Governing definition or originating source</strong><code>{control.sourcePath}</code><p>This defines the rule or origin Server Guy is applying. It does not prove the current result; that comes from the cited evidence.</p></div>}</section>
        {control.evidenceDetails && <section className="evidence-reference-section"><span className="drawer-label">Verify with evidence</span><dl className="evidence-metadata"><div><dt>Observed by</dt><dd>{control.evidenceDetails.source}</dd></div><div><dt>Observed at</dt><dd>{control.evidenceDetails.observedAt}</dd></div><div><dt>Method</dt><dd>{control.evidenceDetails.method}</dd></div><div><dt>Raw result</dt><dd>{control.evidenceDetails.rawResult}</dd></div></dl><button type="button" className="source-link evidence-artifact-link" onClick={() => setRawOpen((value) => !value)}><span className="source-link-copy"><small>Captured artifact</small><strong>{control.evidenceDetails.artifactId}</strong></span><span>{rawOpen ? "Hide raw" : "Open raw"}</span></button>{rawOpen && <pre className="raw-evidence">{control.evidenceDetails.rawEvidence}</pre>}<p className="evidence-scope">This evidence supports <strong>{control.evidenceDetails.parentTitle}</strong>. It establishes only what this observation measured at the recorded time.</p></section>}
        {control.evidence.length > 0 && <section><span className="drawer-label">Verify with evidence</span>{control.evidence.map((event) => <EvidenceRow key={`${event[0]}-${event[1]}`} event={event} parentTitle={control.title} onReviewEvidence={onReviewEvidence} />)}</section>}
        <section className="control-path-section"><span className="drawer-label">Take control</span><button type="button" className="control-path" onClick={() => setTakeoverOpen((value) => !value)}><strong>Take control</strong><small>Inspect and change the underlying source yourself or with another coding agent.</small></button>{takeoverOpen && <div className="takeover-detail"><span>Change this source</span><strong>{control.takeover}</strong><ol><li>Open the source and make the change.</li><li>Return here when the source is ready.</li><li>Re-run verification; do not override the result.</li></ol></div>}</section>
        <section className="rerun-section"><span className="drawer-label">Refresh verification</span><p>{control.rerunHelp}</p><button type="button" className="rerun-button" onClick={rerun} disabled={rerunState === "running"}>{rerunState === "running" ? "Checking current source…" : "Re-run verification"}</button><div className={`rerun-result ${rerunState}`} aria-live="polite">{rerunState === "idle" && "No manual status override is available."}{rerunState === "running" && "Reading the current source and collecting fresh evidence."}{rerunState === "complete" && `Rechecked just now. Result remains “${control.status}” until the underlying source or evidence changes.`}</div></section>
      </aside>
    </div>
  );
}
...
function gateControl(phase, current, gateProgress, { check, checkIndex, passed, currentBlocker }) {
  ...
  return {
    ...
    sourcePath: "docs/user-journeys/01-application-launch.md#uniform-gate-check-contract",
    takeover: phase.takeover,
    evidence: current.events,
...
function evidenceControl(event, current, parentTitle) {
  ...
  return {
    ...
    sourcePath: "docs/user-journeys/01-application-launch.md#observability-link-contract",
