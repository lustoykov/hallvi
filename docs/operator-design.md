...
**The boundary is about commands Pi writes.** The controller runs a small number of commands of its own: the machine check on the connections card, the liveness check on the way in, and the access-log follow behind Overview. Each is fixed text in the controller, only reads, and takes at most a value whose shape cannot carry shell (a container name or an absolute path from an `access-log` record). There is no model-authored text in them for a mode to approve, so they do not prompt and leave no execution record. This is not a fourth mode and not an exception to the three: anything Pi composes still goes through the boundary, including the work of turning access logging on. A new controller command has to meet all three conditions or it belongs to Pi.
