...
**The worker owns Pi's sessions.** It is the only process that opens one. The app asks it over a Unix socket beside the database (`worker.sock`, readable only by its user): for the transcript, to send, to continue, to stop, and to remove an application's histories. The socket is also what makes it the only owner: requests reach whoever holds that path, sessions are opened only on request, and a second worker that finds the path answered leaves. A path nobody answers was left by a worker that died, and is replaced. There is no worker lock, no per-application lock and no heartbeat file.
