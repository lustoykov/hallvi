...
**Conversation control is Pi's.** Send next and Steer are Pi's own durable follow-up and steering queues on an `AgentLane`; Hallvi keeps the accepted message until Pi acknowledges taking it, and records what Pi did. Applications work concurrently. Contextual side-chat opening and concurrent side explanations remain deferred. Existing read-only tool restrictions remain in effect.
...
The composer remains editable during work. **Send next** and **Steer** each save a durable message, and the worker hands it to the conversation's lane in Pi's `AgentHarness` (published `pi-agent-core` 0.85.1). From the moment Pi takes it, the message is Pi's: Pi names it, queues it, orders it, runs it, cancels it and restores it after a restart. Pi decides when it runs: a follow-up when its current work is done, a steer after the tool calls of its current step and before its next model call, one message per turn. A steer does not interrupt a running command or a pending approval, and the interface says so. A message shows as waiting until Pi reads it, at which point it takes its place in the transcript and a new reply opens under it; what Pi did before a steer stays under the reply it happened in.

**The handover.** The API process cannot write into a session the worker owns, so an accepted message is a durable row first. The worker hands it over and records the acknowledgment (`admitted_at`, and the id Pi keeps it under): an idle lane takes it as a prompt whose operation id is the message's own id, a busy lane takes it as a follow-up or a steer and returns its entry id at once. Every message handed over carries its Hallvi id. Hallvi only ever hands over a message it has no acknowledgment for. When a conversation is opened, the worker asks Pi what it already holds — its open operation, its queue, and for unacknowledged messages its history — and adopts that instead of handing anything over twice. That is the whole of the reconciliation between the two, and it covers the instant between Pi taking a message and Hallvi recording that it had. A message queued on a lane that went idle as it arrived is taken back by its id and put as the prompt.

Hallvi schedules nothing inside a conversation. The worker only decides whether one may start: never two for one application, and never two on one server address (see Shared servers below). Pi's session repository does not stop a second process from opening the same history, so the worker lock and the per-application lock remain what makes one writer true.

**Stop** is recorded at once, by the API: the reply being written is settled as stopped and every waiting message as not started, with its text kept. The worker then calls Pi's `abort`, which ends the operation and empties Pi's queues itself. There is no taking back a single waiting message in this version; Stop is the cancellation control, and a message that was not started offers Send again.

**A worker going away** — killed or shut down — aborts nothing in Pi. Its replies are marked interrupted, saying that whether the last command finished is not known, and nothing runs because a worker came back. Every accepted instruction stays visible: a message Pi holds still shows as waiting, under the same id, with the words that it runs when the owner continues. The transition is the owner's:

- **The owner sends a message.** The worker opens the conversation, Pi restores the operation it had open, and Pi resumes it. An interrupted tool call is not made again: Pi gives the model an error result saying the outcome is unknown, and the model investigates. What Pi held queued then runs in its order, and the owner's new message after it. What Pi writes from there is a new reply under the same message.
- **The owner presses Stop.** The interrupted reply and everything waiting are settled as stopped and not started. When the conversation is next opened, the worker ends Pi's open operation and cancels by id anything Pi still has queued, before anything runs.

**Tools, authentication, compaction and retry are Pi's.** Hallvi hands the harness coding-agent's own `ModelRuntime` for credentials, refresh and model access; the same tool definitions as before, with `execute` delegated to Hallvi's workspace and permission wrapper; and nothing at all for compaction and retry, which the harness does by itself. A definition's `prepareArguments` and `constrainedSampling` are fields the harness carries and pass through unchanged. `executionMode` does not: the harness has one setting for a whole turn's tool calls and does not read a tool's own. Every call that changes a server, a file or a record was sequential, so the harness is set to run tool calls one at a time. The cost is that read-only calls in one turn no longer overlap; nothing that mutates can ever run concurrently.

**Shared servers.** The product topology is one stack on one instance, but nothing establishes that an instance belongs to one application, and two applications can be attached to the same address. Their operators would otherwise work on one machine at once, so a conversation does not start while another application on the same address has one live; the message waits. Prohibiting a shared server outright would remove this one remaining gate and is a product decision, not taken here.

```mermaid
flowchart TD
  Send[Send, Send next or Steer] --> Saved[Durable waiting message]
  Saved -->|Worker hands it over, tagged with its id| Lane[Pi AgentLane: prompt, follow-up or steer]
  Lane -->|Entry id| Ack[Acknowledged: Pi owns it now]
  Lane -->|Pi reads it| Delivered[Delivered; a reply opens under it]
  ...
  Stop -->|Worker| Abort[Pi abort: ends the operation, empties its queues]
  Gone[Worker goes away] --> Held[Pi keeps operation and queue; replies interrupted; nothing runs]
  Held -->|Owner sends a message| Resume[Pi resumes; interrupted tool not re-run]
  Held -->|Owner presses Stop| Abort
```

**Upgrading an existing installation.** `npm run db:upgrade` upgrades a schema-15 database in place and keeps the original beside it as `<database>.before-v17`; `db:push` never alters existing data. A conversation's earlier history (`pi-sessions/<application>/<chat>.jsonl`) is opened through Pi's public `JsonlSessionRepo`, which reads that format: on first use it is copied into the conversation's own directory (`pi-sessions/<application>/<chat>/`), where Pi rewrites the copy in its current format. The original file is never modified. Rollback is: stop the app and worker, put the database copy back, run the earlier version; it reads the original histories where they always were. Messages sent after the upgrade exist only in the new copies.
