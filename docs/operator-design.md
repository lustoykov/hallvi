...
**Reading a conversation needs nothing of Pi's runtime.** When nobody is running it, the worker opens Pi's stored session, reads the branch, the lane's own state and how its operations ended, and projects the page from that: no model runtime, no credentials, no workspace, and no change to the model or the tool list. An expired ChatGPT login stops new work, not the history of old work.
