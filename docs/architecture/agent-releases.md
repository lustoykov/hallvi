...
This slice does not implement automatic reconciliation after SSH loss, compatible rollback, migration orchestration, zero-downtime replacement, multiple independent source builds, arbitrary Compose import, BYOM adoption or a plugin runtime. Build directories accumulate for inspection; automated retention is not implemented. Existing schema limitations remain visible capability limits, not permanent product policy. The next useful expansion is broader service/build representation, with the same scoped execution and feedback loop.
