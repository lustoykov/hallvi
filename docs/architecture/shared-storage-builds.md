...
This does not promise arbitrary Compose import, multiple public ports, automatic file ownership repair or multi-host storage. [Generalized backup capture](generalized-protection.md) now admits recorded databases and volumes, including shared mounts and independent builds. Support depends on the declared capture procedure and all writers, not on the primary image name.
