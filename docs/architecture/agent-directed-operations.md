...
`deployment-planner.ts` inspects a pinned repository and submits a validated plan. `deployment-executor.ts` emits one primary HTTP service, built from source or a pinned public Docker Hub image, with optional private PostgreSQL, up to five additional private services with their own builds or shared/published images, shared named volumes and read-only configuration mounts. It prepares a fresh Hetzner host and checks the resulting application. It cannot faithfully represent arbitrary Compose stacks yet; required unsupported services must block planning rather than disappear. Multiple public web/API endpoints and existing-machine adoption remain narrower than the product target. See [shared storage and builds](shared-storage-builds.md) for the current capability and proof.
