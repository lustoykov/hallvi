...
- User access tokens normally expire after eight hours. This prototype currently discards the refresh token, so it requires sign-in again. That is an implementation gap, not a device-flow restriction: [GitHub supports refreshing device-flow user tokens without a client secret](https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/refreshing-user-access-tokens). The refresh token lasts six months and rotates when used. Automatic refresh is a follow-up; expired credentials never silently fall back to the CLI or another account.
