...
This is the GitHub connection and repository-access reference. Server Guy reads repositories; it does not write to them or open pull requests. The [product boundary](../../PRODUCT.md#operating-boundary) limits any future application-code proposal to narrow operability changes the owner merges.
...
- Own configuration: `.server-guy/github-connection.json`, or `SERVER_GUY_CONFIG_DIR/github-connection.json`. Writes are atomic with file mode `0600`. Access and refresh tokens are unencrypted at rest; do not expose this local prototype to the network or share its state directory. Neither token is included in browser data, evidence or provider-error messages.
...
The browser fixture replaces only GitHub's API/credential boundary. The real GitHub setup routes, coordinator, domain code and SQLite run in the fixture. Pi and ChatGPT login use their existing synthetic adapters. CI never authorizes a real account. Earlier local/live evidence is recorded in the [archived acceptance guide](https://github.com/lustoykov/server-guy/blob/0682ab257469bc5cee994572285283ea949bc3c6/docs/archive/implementation/phase-one-acceptance.md#latest-verification).
