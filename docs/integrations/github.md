...
- Own configuration: `github-connection.json` in the account directory — `~/.config/hallvi/pi`, or `HALLVI_PI_CONFIG_DIR`, or `HALLVI_CONFIG_DIR` when only that is set — beside the ChatGPT login, shared by every controller on the machine that uses that directory and never copied between them. Writes are atomic with file mode `0600`. Access and refresh tokens are unencrypted at rest; do not expose this local prototype to the network or share its state directory. Neither token is included in browser data, evidence or provider-error messages.
