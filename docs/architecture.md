...
A command check may change data, so its outcome is never guessed either. The host runs it detached from the SSH session and writes its exit status and output beside the attempt's host result; when the container has `timeout`, the command runs under it, so a time limit stops the command instead of abandoning it. Before the command starts, the deployment records the pending command; a lost session, a missing record, a still-running command or a limit the container could not enforce leaves that hold, and nothing executes or verifies again under the operation that ran it. `reconcile_release` reads the host's record and turns the hold into a known pass (verification resumes without another replacement), a known failure (correction within the scope), a command that never started, or an explicit unresolved hold. Only the owner's retry of that work, or a new approval whose text names the unknown, accepts that the command may run again.
