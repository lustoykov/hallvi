# Implementation history and retained acceptance backlog
...
Archived from ROADMAP.md on 2026-09-08 during plan consolidation. This is historical implementation evidence and retained unfinished acceptance work, not the active build sequence. [ROADMAP.md](../../../../../ROADMAP.md) owns all new delivery order and scope. Unchecked acceptance conditions are not waived by moving them here; roadmap increment 1 must resolve or explicitly carry them forward. Older AWS, provider and controller proposals are subject to the current [Product scope](../../../../../PRODUCT.md#supported-scope).

## Development sequence

Historical implementation sequence and retained backlog. The priority above governs new work. Checked items below describe earlier slices and their evidence, not completion of the updated experience or remote deployment. Conditional technology adoption is not a mandatory step.
...
| 5 | Connect GitHub explicitly: authorization, scope, revocation, exact repository access. | Merged: [PR #12](https://github.com/lustoykov/server-guy/pull/12). [Checklist](#connect-github-explicitly), [setup and boundaries](../../../../integrations/github.md). |
| 6 | Make Pi requests durable: SQLite, one local Node worker, run IDs, revisioned messages, reconnectable SSE, bounded transcript and durable summary. | Merged: [PR #13](https://github.com/lustoykov/server-guy/pull/13). Synthetic process and desktop verification; real-Pi streaming remains opt-in. [Contract](../../../implementation/durable-pi-requests.md), [checklist](#make-pi-requests-durable). |
| 7 | Keep application Activity meaningful, with bounded local diagnostic logs and optional OpenTelemetry export. | Merged: [PR #16](https://github.com/lustoykov/server-guy/pull/16), including the September 6 correction that removes Reply details and detailed SQLite diagnostics. Extends alongside later Operations. [Checklist](#action-history-and-tracing), [small spec](../../../implementation/action-history-and-tracing.md). |
| 8 | Let Pi retrieve application status and recorded evidence through a scoped read-only tool instead of injecting the full summary before every reply. | Merged: [PR #17](https://github.com/lustoykov/server-guy/pull/17). [Spec](../../../implementation/application-status-tool.md), [checklist](#application-status-lookup). |
| 9 | Implement the Phase 2 Application Contract as a read-only vertical slice. | Consolidated into open PR #21 on `codex/phase23-core-followups`; not merged. Started 2026-09-06 by the user's decision before Phase 1 human sign-off; the Phase 1 gaps below stay open. [Spec](../../../implementation/application-contract.md), [checklist](#phase-2-application-contract), [product contract](../user-journeys/01-application-launch-phase-reference.md#phase-2--inspect-app), [learning exercises](../../../../learning/stack-with-server-guy.md#the-first-learning-slice-phase-2-application-contract). |
| 9b | Implement Phase 3, Make launch-ready: the conformance brief, Pi's staged changes with isolated previews, publication under the Approval Mode, the exact merged candidate and Server Guy's own conformance run in disposable containers. | Consolidated into open PR #21 on `codex/phase23-core-followups`; not merged. Docker Engine on the controller host is the accepted prerequisite for built-in execution (decided 2026-09-06). [Spec](../../../implementation/phase-three-conformance.md), [checklist](#phase-3-conformance-result), [product contract](../user-journeys/01-application-launch-phase-reference.md#phase-3--make-launch-ready). |
...
| 16 | Complete the home-server, managed-platform, Python, and AWS ECS/Fargate transfer labs without expanding Server Guy's V1 boundary. | Later learning work: [home-server constraints](#home-server-controller-mode), [learning guide](../../../../learning/stack-with-server-guy.md), [AWS direction](#aws-integration-direction). |

Merge status above and checkbox status below are distinct: a checked item is implemented in this branch, not proof of complete phase acceptance. Test results and unresolved acceptance evidence live in the [testing guide](../../../implementation/phase-one-acceptance.md#latest-verification).

Merged follow-ups: [native Pi sessions and permission boundaries](../../../implementation/native-pi-and-permissions.md) ([PR #15](https://github.com/lustoykov/server-guy/pull/15): native per-Chat history replaces custom replay/compaction, saved Decisions are retrieved through a scoped read-only tool, and invalid proposals return in-loop feedback), action history and tracing, and the [application status lookup](#application-status-lookup) that replaces the per-reply application summary. External authority and atomic Decision commits are unchanged; independently committed tools remain deferred. Phase 2 and Phase 3 are consolidated into open PR #21; see the retained [checklist](#phase-2-application-contract).

## Phase 1 implementation backlog

**September 6 acceptance:** the merged implementation passed 29 live automated cases, a six-answer targeted follow-up after an approval-wording clarification, all 26 desktop tests, and 560 deterministic tests. Codex reviewed the saved answers separately from human verdicts. [Evidence and remaining coverage](../../../implementation/phase-one-acceptance.md#september-6-acceptance-pass-after-prs-1517-merged). Human meaning sign-off and the explicitly listed end-to-end coverage gaps remain open; this is not a new infrastructure milestone.

Finish the required [Phase 1 acceptance gates](../../../implementation/phase-one-acceptance.md#what-done-means), including explicit GitHub connection and durable requests, before Phase 2 merges. **Decision 2026-09-06:** Phase 2 development started on `codex/phase-two-application-contract` before human sign-off, at the user's request; its PR stays unmerged until that sign-off or an explicit waiver, and the unchecked items below are neither closed nor rewritten by it. Optional cleanup and conditional architecture studies are not automatic blockers. Server Guy uses Pi as its only model and agent runtime; do not add AI SDK Core or `useChat` to the product.
...
- [x] Promote prototype A to the real login screen; keep technical details in a separate help panel and preserve the current connection until replacement login succeeds. See the [current setup rules](../../../implementation/phase-one-acceptance.md#current-entry-points-and-setup-rules).
...
- [x] Return from Pi setup to an [applications overview](../../../implementation/phase-one-acceptance.md#current-entry-points-and-setup-rules), with explicit creation and switching; keep Pi configuration installation-wide and each application's chats, Decisions, and checks separate.
...
- [x] Document the [Phase 1 desktop acceptance contract](../../../implementation/phase-one-acceptance.md), separating deterministic UI/domain tests from live Pi behavior, GitHub authorization and pending durable-worker gates.
...
- [ ] Extend checked-in browser coverage to remaining branches of the twenty-case [acceptance contract](../../../implementation/phase-one-acceptance.md#current-journey-contract); current automation covers selected branches, not complete Phase 1 acceptance.
...
The [implementation contract](../../../implementation/durable-pi-requests.md) defines the first vertical slice and the changed failure semantics. Start with durable acceptance and worker completion, then add recovery/streaming and bounded context; complete the checklist before claiming this milestone done.
...
Give users application history for meaningful domain events and ordinary Chat with safe recovery. Operators debug using bounded local event logs and completed spans, with optional trace export. The [action history and tracing spec](../../../implementation/action-history-and-tracing.md#application-activity-inclusion-rules) owns inclusion rules and the flow inventory; build order and completion stay here. This adds no Phase 1 acceptance gate.
...
- [x] Extend the synthetic real-Pi casebook with seven `Application status` cases (acknowledgement and conceptual questions without a lookup; blockers, recorded access versus a live recheck, stale checks-passed history, a changed Approval Mode, and readiness versus deployment) and status-lookup expectations on the greeting and GitHub cases; run and review them on explicit opt-in, as recorded in the [testing guide](../../../implementation/phase-one-acceptance.md#latest-verification).
- [x] Update native-session documentation to reflect the tool and the reduced Run context.

## Phase 2 Application Contract

Read-only vertical slice on `codex/phase-two-application-contract`; the [spec](../../../implementation/application-contract.md) owns the contract, the [journey](../user-journeys/01-application-launch-phase-reference.md#phase-2--inspect-app) the gates, the [testing guide](../../../implementation/phase-one-acceptance.md#phase-2-acceptance-inspect-app) the cases and evidence. Confirmed direction: Phase 2 establishes the contract and records known conformance work for Phase 3; unknown required values still block; open policies stay visible until their later gates.
...
Vertical slice on `codex/phase-three-conformance`, stacked on the Phase 2 branch; the [spec](../../../implementation/phase-three-conformance.md) owns the contract, the [journey](../user-journeys/01-application-launch-phase-reference.md#phase-3--make-launch-ready) the gates, the [testing guide](../../../implementation/phase-one-acceptance.md#phase-3-acceptance-make-launch-ready) the cases and evidence. Confirmed direction: Docker Engine on the controller host is the prerequisite for built-in execution; static checks are previews; Server Guy publishes and the engineer merges; the candidate is the observed merged revision.
...
Build order and status stay here. The boundary between an EC2 host and an ECS/Fargate deployment target, capability details, and supporting references live in [docs/integrations/aws.md](../integrations/aws.md).
