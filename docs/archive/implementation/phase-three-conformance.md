...
Status: consolidated on `codex/phase23-core-followups`, including the Phase 2, Phase 3 and experience follow-ups. This document is the implementation contract for Phase 3: the transition, the conformance brief, Pi's staged changes and isolated previews, approval and publication, external returns, the exact candidate, the versioned check set and its runner, the Docker prerequisite, and the boundaries this slice deliberately leaves alone. The [journey](../previous-direction/docs/user-journeys/01-application-launch-phase-reference.md#phase-3--make-launch-ready) owns the product behavior and gate definitions; [ROADMAP.md](../previous-direction/docs/plans/implementation-history.md#phase-3-conformance-result) owns build order and status; the [testing guide](phase-one-acceptance.md#phase-3-acceptance-make-launch-ready) owns acceptance cases and dated evidence.
