...
| P1-20 Durable replies | Submit twice with one request key; leave/reopen the chat; cancel and retry; crash/restart the worker; queue another Chat; exceed context budgets. | Acceptance returns HTTP 202 with stable IDs. One worker serializes requests. Reconnect reads saved revisions; failure/cancellation saves no Decisions. A restart marks running attempts interrupted, never silently repeated. Summary coverage advances only after a successful bounded summary; current Decisions are never silently dropped. |
