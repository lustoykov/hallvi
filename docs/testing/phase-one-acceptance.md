...
- **Phase 1 product work is complete:** the current journey below passes, plus the explicit GitHub connection and durable-request follow-ups in the [roadmap](../../ROADMAP.md#phase-1-implementation-backlog) pass their acceptance cases. GitHub connection has automated and live happy-path evidence; durable requests have synthetic process and browser coverage, while live model verification and broader acceptance work remain. Do not treat the four checks as evidence that the entire product phase is done.
...
The runnable [casebook](../../tests/evals/phase-one-cases.ts) contains eighteen fixed inputs and meaning rubrics: the original eight Decision/instruction cases, three GitHub evidence cases, and seven native conversation cases. The native cases cover old active records, cross-chat supersession, correction versus addition, implicit constraints and cancellation before/after compaction. They seed synthetic earlier history/usage and reduce retained context for inexpensive real native compaction; replies, tool lookup and summaries are real. This is a small behavioral regression set, not a production context-capacity or broad reliability benchmark.
...
```sh
# Eighteen real Pi turns using your saved Server Guy model/effort and subscription.
