...
| P1-20 Durable replies | Submit twice with one request key; leave/reopen the chat; cancel and retry; crash/restart the worker; queue another Chat; compact history; rebuild a missing native session. | Acceptance returns HTTP 202 with stable IDs. One worker serializes requests and retains writer locks until SDK settlement. Reconnect reads saved revisions; failure/cancellation saves no Decisions. A restart marks running attempts interrupted, never silently repeated. Native compaction preserves the conversation branch; scoped lookups read current Decisions. Rebuild preserves saved records, never sends an automatic reply. |
