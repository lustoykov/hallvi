...
| --- | --- |
| [BookStack follow-ups, 11 September](2026-09-11-bookstack-followups.md) | Real-Pi replay on the audit's records. Given only the symptom, a new conversation diagnosed the failed administrator setup from fresh logs, the deployed bootstrap and upstream source. The correction Pi proposed passed a recorded command check that proved the owner's login and rejected the defaults. Local Docker with stand-in GitHub, Hetzner and SSH; backup and isolated restore on a Linux host container with TLS MinIO are recorded in the document. |
