...
The next GitHub run (`34335493870`) passed deployment and the other two smoke cases. The remaining conversation test started its ten-second draft assertion while the first-use chat-creation POST was still pending and Next was rebuilding. The test now waits for that exact POST to return HTTP 201 using the existing 30-second HTTP acceptance budget, then applies the unchanged UI assertion. This adds an explicit creation check rather than treating an unfinished request as a broken draft. Its fresh isolated local rerun passed in 28.6 seconds; lint/formatting and all 550 documentation links were checked. Run `34336258845` then confirmed creation succeeded (22.4 seconds) but the first chat-selection GET was still pending when the next draft assertion began. The journey now checks HTTP creation, selection and message acceptance separately (30 seconds each), verifies the selected chat identity, and keeps UI assertions at ten seconds. Its total budget is 180 seconds to accommodate multiple cold handlers. No product code or coverage was changed in these test corrections.

Run `34337064226` showed compilation variability exceeding the 30-second chat HTTP budget itself. The disposable browser fixture now warms the four conversation handlers with read-only GETs against nonexistent IDs before tests start; it creates no application, chat or run. Warm-up belongs to bounded fixture startup (180 seconds), while test HTTP/UI limits remain explicit. Server errors during warm-up fail setup. This avoids treating development compilation as a draft-state failure without removing any journey or using automatic retries. All four smoke cases passed locally after warm-up. The owner then asked to leave CI running and pause investigation; no passing final CI or merge is claimed here.
