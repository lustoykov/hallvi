...
Pi is Server Guy's only model and agent runtime. The [explicit Pi setup](../../TODO.md#2-configure-pi-explicitly) detects existing provider/model/effort and credential presence without using them. The user chooses whether to reuse that shared credential store and snapshot the displayed preferences, or configure separately with `openai-codex` / `gpt-5.6-sol` / `high` and a Server Guy-owned OAuth file. Existing API-backed setups require a billing acknowledgment. Server Guy does not import machine tools/extensions/instructions, copy Codex CLI credentials, overwrite global Pi model settings, or automatically fall back to API billing. The setup screen checks local configuration; provider validity and quota are checked on send.
